...
- **Key Mismatch (Site vs. Secret):**
  ...
  - The **Secret Key** (private) is used _only_ on your server-side API route for verification. Accidentally swapping them or exposing the secret key on the client-side will cause failure.
...
**Step 2: Test an _Unlisted_ Domain (The Negative Test)**
...
**By default, no.** reCAPTCHA v3 is designed to be invisible and unobtrusive. A successful load is silent. The absence of errors _is_ the sign of success.
