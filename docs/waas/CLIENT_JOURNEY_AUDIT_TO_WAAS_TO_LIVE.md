...
- Persists client_selected_template_slug in tenant_site_config (PR #94 — picks from 10 templates).

5. Step 5 Integrations

- Saves Calendly/USP/financing/functionality preferences.
- Does not finalize onboarding — advances to Step 6.

6. Step 6 Website Builder

- Customer edits pre-filled section content/blocks (`saveOnboardingStepBuilder`).
...
- onboarding -> pending_review (fires at Step 6 completion, not Step 5)
...
Non-active tenants (onboarding / pending_review / suspended / cancelled) render a
branded "coming soon" placeholder instead of a bare 404 if their runtime URL is
visited early — see `components/waas/SiteComingSoon.tsx`.
