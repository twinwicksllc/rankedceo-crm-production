    ...
    >
      ...
      <div className="flex flex-col gap-1 p-1.5">
        {enabledSections.map((s, idx) => {
          ...
          const heroStyle = s.section === 'hero'    ? { backgroundColor: primary  + '44' } : {}
          const ctaStyle  = s.section === 'booking' ? { backgroundColor: accent   + '55' } : {}
          const svcStyle  = s.section === 'services'? { backgroundColor: secondary+ '33' } : {}
...
function PaletteSwatch({ palette }: { palette: SiteTemplate['preview_palette'] }) {
  return (
    <div className="flex h-2 w-full rounded-full overflow-hidden gap-px">
      <div className="flex-[2]" style={{ backgroundColor: palette.primary }} />
      <div className="flex-[2]" style={{ backgroundColor: palette.secondary }} />
    ...
    >
      ...
      {isSelected && (
        <div className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-violet-500 flex items-center justify-center shadow-sm z-10">
          <svg className="w-3 h-3 text-white" fill="none" viewBox="0 0 12 12" strokeWidth={2.5} stroke="currentColor">
