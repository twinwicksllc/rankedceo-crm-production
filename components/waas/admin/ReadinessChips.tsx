...
export function getTenantReadiness(
  ...
  const profile: ReadinessCheck =
    hasName && hasTrade
      ? {
          ...
          label: "Profile",
          status: "ready",
          detail: `${tenant.legal_name} · ${tenant.primary_trade}`,
        }
      : hasName || hasTrade
        ? {
            key: "profile",
            ...
            status: "partial",
            detail: hasName
              ? `Has name, missing trade`
              : `Has trade, missing name`,
          }
        : {
            key: "profile",
            label: "Profile",
            status: "missing",
            detail: "No business name or trade set",
          };
  ...
  const onboarding: ReadinessCheck = tenant.onboarding_completed
    ? {
        ...
        label: "Onboarded",
        status: "ready",
        detail: tenant.onboarding_completed_at
          ? `Completed ${new Date(tenant.onboarding_completed_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`
          : "Onboarding complete",
      }
    : tenant.onboarding_step > 0
      ? {
          ...
          label: "Onboarded",
          status: "partial",
          detail: `In progress — step ${tenant.onboarding_step}`,
        }
      : {
          key: "onboarding",
          label: "Onboarded",
  ...
  const domain: ReadinessCheck = hasDomain
    ? {
        ...
        label: "Domain",
        status: "ready",
        detail: tenant.domain ?? undefined,
      }
    : hasSubdomain
      ? {
          key: "domain",
          label: "Domain",
          status: "partial",
          detail: `Subdomain: ${tenant.subdomain}.rankedceo.com`,
        }
      : {
          key: "domain",
          label: "Domain",
  ...
  const dns: ReadinessCheck = tenant.domain_verified
    ? {
        ...
        label: "DNS",
        status: "ready",
        detail: tenant.domain_verified_at
          ? `Verified ${new Date(tenant.domain_verified_at).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`
          : "DNS verified",
      }
    : hasDomain
      ? {
          ...
          label: "DNS",
          status: "partial",
          detail: "Domain set but DNS not verified yet",
        }
      : {
          key: "dns",
          label: "DNS",
          status: "missing",
          detail: "No domain to verify",
        };
  ...
  const template: ReadinessCheck = Boolean(tenant.client_selected_template_slug)
    ? {
        ...
        label: "Template",
        status: "ready",
        detail: tenant.client_selected_template_slug ?? undefined,
      }
    : {
        key: "template",
        label: "Template",
        status: "missing",
        detail: "No template selected by client",
      };
  ...
  const payment: ReadinessCheck = Boolean(tenant.stripe_subscription_id)
    ? {
        ...
        label: "Paid",
        status: "ready",
        detail: `Sub: ${tenant.stripe_subscription_id}`,
      }
    : {
        key: "payment",
        label: "Paid",
    ...
    return (
      ...
      >
        {checks.map((check) => (
          ...
          >
            <span
              className={`w-1.5 h-1.5 rounded-full shrink-0 ${DOT_COLORS[check.status]}`}
              aria-hidden="true"
            />
            {check.label}
          </span>
  ...
  return (
    ...
    >
      {checks.map((check) => (
        ...
        >
          <span
            className={`w-1 h-1 rounded-full shrink-0 ${DOT_COLORS[check.status]}`}
            aria-hidden="true"
          />
          {check.label}
        </span>
