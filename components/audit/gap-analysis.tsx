...
export function GapAnalysis({ gapAnalysis, targetDomain }: GapAnalysisProps) {
    ...
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 10, flexWrap: 'wrap' }}>
          <StatBadge value={criticalCount} label="Critical Gaps" color="#EF4444" />
          <StatBadge value={warningCount}  label="Warnings"      color="#F59E0B" />
          <StatBadge value={gapAnalysis.missingKeywords.length} label="Missing Keywords" color="#A78BFA" />
          <StatBadge value={gapAnalysis.opportunityScore}       label="Opportunity Score" color="#22C55E" suffix="/100" />
        </div>
      </div>
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {gapAnalysis.missingKeywords.map((gap, i) => (
              <GapRow key={i} gap={gap} type="missing" />
            ))}
          </div>
        </div>
      )}
...
function GapRow({ gap, type }: { gap: KeywordGap; type: 'missing' | 'gap' }) {
