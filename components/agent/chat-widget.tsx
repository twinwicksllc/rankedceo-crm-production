...
export function ChatWidget({
  ...
  async function sendMessage(text?: string) {
    ...
    try {
      ...
      const actionIsBooking =
        data.action === "show_booking" || data.action === "booking_confirmed";
      ...
      if (triggerBookingTrue && actionIsBooking && data.calendlyUrl) {
        console.error("[FINAL-CHECK] REDIRECT TRIGGERED BY:", {
          triggerBooking: data.triggerBooking,
          action: data.action,
        });
        ...
        console.error(
          "[FINAL-CHECK] NOT REDIRECTING - waiting for explicit booking intent",
        );
      }
  ...
  return (
    <div className={`fixed ${positionClass} z-50`} style={colorStyle}>
      ...
      {isOpen && (
        <div
          className="mb-4 w-80 sm:w-96 rounded-2xl shadow-2xl border border-gray-200 bg-white overflow-hidden flex flex-col"
            ...
            <>
              ...
              {messages.length === 1 && (
                <div className="px-4 py-2 flex gap-2 flex-wrap border-t border-gray-100 bg-white">
                  {["I need help", "Book a call", "Get a quote"].map(
                    (reply) => (
                      <button
                        key={reply}
