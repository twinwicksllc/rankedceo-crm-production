...
const HandoffStepSchema = BaseStepSchema.extend({ type: z.literal('handoff'), from: PersonaSchema, to: PersonaSchema, message: z.string(), handoff_timeout_ms: z.number().optional() })
const PauseStepSchema = BaseStepSchema.extend({ type: z.literal('pause'), duration_ms: z.number() })
