...
export class EmailAdapter {
  ...
  async verifyEmailSent(verification: EmailVerification): Promise<void> {
    if (this.mode === "skip") {
      console.log(
        `  [EmailAdapter] Skip mode — not verifying email to ${verification.to}`,
      );
      return;
    }
    ...
    if (!apiKey)
      throw new Error("RESEND_API_KEY not set — cannot verify email delivery");
    ...
    console.log(
      `  [EmailAdapter] Polling Resend logs for email to ${to} (subject: "${subjectContains}")...`,
    );
  ...
  private async pollResendLogs(
    ...
    const data = (await res.json()) as {
      data: Array<{ to: string[]; subject: string }>;
    };
