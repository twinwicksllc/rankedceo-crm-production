...
export class SupabaseAdapter {
  ...
  constructor() {
    ...
    const url =
      process.env.NEXT_PUBLIC_WAAS_SUPABASE_URL ?? process.env.SUPABASE_URL;
    const key =
  ...
  async countRows(
    ...
    if (error)
      throw new Error(`SupabaseAdapter.countRows error: ${error.message}`);
    return count ?? 0;
  }
  ...
  async insert(table: string, row: Record<string, unknown>): Promise<void> {
    ...
    if (error)
      throw new Error(`SupabaseAdapter.insert error: ${error.message}`);
  }
  ...
  async select(
    ...
    if (error)
      throw new Error(`SupabaseAdapter.select error: ${error.message}`);
  ...
  async purgeAgentRecords(
    ...
    if (error)
      throw new Error(
        `SupabaseAdapter.purgeAgentRecords error: ${error.message}`,
      );
