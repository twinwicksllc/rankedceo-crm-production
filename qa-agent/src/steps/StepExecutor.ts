...
import type { ScenarioStep, Finding, Persona } from "../types.js";
...
export class StepExecutor {
  ...
  private async runStep(step: ScenarioStep): Promise<void> {
    ...
    switch (step.type) {
      ...
      case "assert_url":
        return this.stepAssertUrl(step.persona, step.pattern);
      case "assert_db":
        return this.stepAssertDb(step.table, step.where, step.expected_count);
      case "handoff":
        return this.stepHandoff(
          step.from,
          step.to,
          step.message,
          step.handoff_timeout_ms,
        );
      case "pause":
        return this.stepPause(step.duration_ms);
      default: {
        // TypeScript exhaustive check
        const _exhaustive: never = step;
        throw new Error(
          `Unknown step type: ${(_exhaustive as ScenarioStep).type}`,
        );
      }
    }
  }
  ...
  private async stepWaitFor(
    ...
    try {
      ...
      throw new Error(
        `wait_for timeout for selector "${selector}" at "${currentUrl}": ${msg}`,
      );
    }
  }
  ...
  private async stepWaitForUrl(
    ...
    try {
      ...
      const isSimplePath =
        pattern.startsWith("/") && !/[\^$()[\]{}|+*?]/.test(pattern);
      if (!isSimplePath) {
        throw firstErr;
      }
      ...
      try {
        ...
        await page.waitForFunction(
          (regexSource) =>
            new RegExp(regexSource).test(window.location.pathname),
          pathRegex.source,
          { timeout: timeoutMs },
        );
        ...
        if (currentUrl.includes("/login")) {
          try {
            const alertText = await page
              .locator(
                '[role="alert"], [data-testid="login-error"], .text-destructive',
              )
              .first()
        ...
        throw new Error(
          `wait_for_url timeout for pattern "${pattern}" at "${currentUrl}": ${msg}${loginErrorText}`,
        );
      }
    }
  }
  ...
  private async stepAssertUrl(
    ...
    if (!re.test(pathname)) {
      throw new Error(
        `assert_url failed: pattern "${pattern}" did not match pathname "${pathname}" (url: "${url}")`,
      );
    }
  }
  ...
  private async captureDomSnippet(
    persona: Persona,
  ): Promise<string | undefined> {
    try {
      const page = await this.router.getPage(persona);
