...
import type { SelfHealPayload, SelfHealProposal, SelfHealConfig } from './types.js'
...
export async function relocate(
  ...
  if (config.provider === 'stub') {
    ...
    console.log('[llm-relocate] See docs/qa-agent/self-healing.md for full guide.')
...
export async function relocateFromIssue(
  ...
  if (!issueBody) {
    console.warn(`[llm-relocate] Could not fetch body of issue #${issueNumber}`)
    return null
  }
...
function extractPayload(issueBody: string): SelfHealPayload | null {
  ...
  try {
    ...
    console.warn('[llm-relocate] Failed to parse selfHealPayload JSON:', jsonStr.slice(0, 200))
    return null
  }
}
...
async function callLlm(
  ...
  if (config.provider === 'anthropic') {
    ...
    console.warn('[llm-relocate] Anthropic provider selected but not yet wired. See docs/qa-agent/self-healing.md')
    return null
  }
...
if (process.argv[1]?.endsWith('llm-relocate.ts') || process.argv[1]?.endsWith('llm-relocate.js')) {
  const args = process.argv.slice(2)
  const issueArg = args.indexOf('--issue-number')
  const repoArg = args.indexOf('--repo')

  if (issueArg === -1 || repoArg === -1) {
    console.error('Usage: npx tsx llm-relocate.ts --issue-number <N> --repo owner/repo')
    process.exit(1)
  }
  ...
  const provider = (process.env.SELF_HEAL_PROVIDER ?? 'stub') as SelfHealConfig['provider']
