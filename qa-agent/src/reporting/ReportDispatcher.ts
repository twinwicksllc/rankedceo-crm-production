...
export class ReportDispatcher {
  ...
  async dispatch(report: RunReport, reportDir: string): Promise<void> {
    ...
    results.forEach((r, i) => {
      ...
      if (r.status === "rejected") {
        console.warn(
          `⚠️  ReportDispatcher: ${channel} failed — ${(r.reason as Error).message}`,
        );
      } else {
  ...
  private async sendEmail(report: RunReport, html: string): Promise<void> {
    ...
    if (!resendKey || !adminEmail) {
      console.warn(
        "[ReportDispatcher] RESEND_API_KEY or QA_ADMIN_EMAIL_NOTIFY not set — skipping email",
      );
      return;
    }
...
<body style="font-family:sans-serif;color:#1a1a1a;padding:24px;max-width:600px">
  ...
  private async postGitHubSummary(report: RunReport): Promise<void> {
    ...
    if (!summaryFile) {
      console.log(
        "[ReportDispatcher] Not in GitHub Actions — skipping summary",
      );
      return;
    }

    const emojiMap2: Record<string, string> = {
      pass: "✅",
      pass_with_findings: "⚠️",
      error: "❌",
      critical_halt: "🚨",
