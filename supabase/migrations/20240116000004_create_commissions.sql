...
DECLARE
    ...
    v_user_id UUID;
BEGIN
    -- Only create commission when deal is marked as won
    IF NEW.stage = 'won' AND (OLD.stage IS NULL OR OLD.stage != 'won') THEN
        -- Get the user who owns the deal (assigned_to or created_by)
...
BEGIN
    -- Only update if deal is won and value changed
    IF NEW.stage = 'won' AND NEW.value != OLD.value THEN
