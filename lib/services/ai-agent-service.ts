...
const SOURCE_CONTEXT: Record<AppointmentSource, string> = {
  ...
  plumbing:
    "plumbing services including pipe repair, drain cleaning, water heater installation, and emergency plumbing",
  electrical:
    "electrical services including wiring, panel upgrades, outlet installation, and electrical repairs",
  smile:
    "dental and smile makeover services including cosmetic dentistry, teeth whitening, and smile assessments",
  ...
  const eventTypeList =
    eventTypes.length > 0
      ? eventTypes
          .map(
            (et) =>
              `- ${et.name} (${et.duration} minutes): ${et.description || "Available for booking"}`,
          )
          .join("\n")
...
{"detected": true or false, "confidence": 0.0 to 1.0, "notes": "brief reason"}`;
  ...
  try {
    ...
    console.warn(
      "[AI Agent] Intent detection failed, falling back to keyword detection:",
      err,
    );
    return { detected: false, confidence: 0 };
  }
...
export function extractLeadInfo(messages: AgentMessage[]): {
  ...
  if (!name && (email || phone)) {
    ...
    const beforeEmail = email
      ? userMessages.substring(0, userMessages.indexOf(email)).trim()
      : "";
    const beforePhone = phone
      ? userMessages.substring(0, userMessages.indexOf(phone)).trim()
      : "";

    // Try to extract name from the text before email/phone
    const nameMatch3 = (beforeEmail || beforePhone).match(
...
export async function chat(
  ...
  const keywordIntent =
    detectBookingIntent(userMessage) || detectBookingIntent(assistantMessage);
