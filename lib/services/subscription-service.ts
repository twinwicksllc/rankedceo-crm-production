...
export class SubscriptionService {
  ...
  async handleWebhookEvent(event: Stripe.Event): Promise<void> {
    ...
    await this.supabase.from("crm_billing_events").upsert({
  ...
  private async syncSubscriptionToDb(
    ...
    if (!userId) {
      console.error(
        "[Stripe] Subscription missing userId metadata:",
        subscription.id,
      );
      return;
    }
    ...
    const { error } = await this.supabase.from("crm_subscriptions").upsert(
      {
        user_id: userId,
