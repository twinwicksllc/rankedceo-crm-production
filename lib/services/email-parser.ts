...
export class EmailParser {
  ...
  static parseFromSendGrid(webhookData: any): ParsedEmail {
    ...
    const parsedEmail: ParsedEmail = {
      ...
      bcc_addresses: webhookData.bcc
        ? this.extractEmails(webhookData.bcc)
        : null,
  ...
  private static extractHeaders(webhookData: any): Record<string, any> {
    ...
    if (webhookData["Message-ID"])
      headers["Message-ID"] = webhookData["Message-ID"];
    if (webhookData["In-Reply-To"])
      headers["In-Reply-To"] = webhookData["In-Reply-To"];
    if (webhookData["References"])
      headers["References"] = webhookData["References"];
