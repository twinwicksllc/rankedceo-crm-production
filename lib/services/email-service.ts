...
export class EmailService {
  ...
  async getEmails(filters?: EmailFilters): Promise<EmailMessageWithThread[]> {
    ...
    if (filters?.search) {
      query = query.or(
        `subject.ilike.%${filters.search}%,from_address.ilike.%${filters.search}%`,
      );
    }
  ...
  async searchEmails(query: string): Promise<EmailMessageWithThread[]> {
    const { data, error } = await this.supabase
      ...
      .or(
        `subject.ilike.%${query}%,body_plain.ilike.%${query}%,from_address.ilike.%${query}%`,
      )
      .order("received_at", { ascending: false });
