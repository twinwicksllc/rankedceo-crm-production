...
export class AppointmentService {
  ...
  async saveCalendlyConnection(connection: {
    ...
    if (error)
      throw new Error(`Failed to save Calendly connection: ${error.message}`);
    return data;
  }
  ...
  async disconnectCalendly(): Promise<void> {
    ...
    if (error)
      throw new Error(`Failed to disconnect Calendly: ${error.message}`);
  }

  // ── Get active Calendly connection for an account (for public booking) ──

  async getAccountCalendlyConnection(
    accountId: string,
