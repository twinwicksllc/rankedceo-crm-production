...
function extractServicesList(report: AuditReportData): string[] | null {
  ...
  if (report.opportunities && Array.isArray(report.opportunities)) {
    report.opportunities.slice(0, 3).forEach((opp) => {
      ...
      const conceptMatches = desc.match(/\b(SEO|content|mobile|performance|speed|user|experience|accessibility)\b/gi)
      if (conceptMatches) {
 ...
 */
export async function extractAuditPreFillForStep1(auditId: string | null | undefined): Promise<AuditPreFillData> {
  if (!auditId) return {}
  ...
  try {
    ...
    if (report.provider_meta?.keyword_detected_location) {
      const { city, state } = parseLocationString(report.provider_meta.keyword_detected_location)
