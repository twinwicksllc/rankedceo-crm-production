...
export function parseMissingTenantColumn(errorMessage: string): string | null {
  const match = errorMessage.match(
    /Could not find the '([^']+)' column of 'tenants' in the schema cache/i,
  );
  return match?.[1] ?? null;
}
...
export function isMissingSchemaTable(
  ...
  const re = new RegExp(
    `Could not find the table 'public\\.${escaped}' in the schema cache`,
    "i",
  );
...
export function isMissingBucketError(
  ...
  const re = new RegExp(
    `bucket.*${escaped}.*(not found|does not exist)|not found`,
    "i",
  );
  return re.test(errorMessage);
}
...
export async function updateTenantWithFallback(
  ...
  for (let attempt = 0; attempt < 12; attempt += 1) {
    ...
    if (
      isPendingReviewEnumError(error.message) &&
      mutablePayload.status === "pending_review"
    ) {
  ...
  return {
    error: { message: "Tenant update failed after schema fallback retries." },
  };
}
...
export async function insertTenantWithFallback(
  ...
  return {
    error: { message: "Tenant insert failed after schema fallback retries." },
  };
}
