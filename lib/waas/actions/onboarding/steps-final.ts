...
export async function saveOnboardingStep4(
  ...
  try {
    ...
    const updatedBrandConfig = {
      ...
      tone: data.tone || null,
      fonts: {
        ...(typeof existingConfig.fonts === "object" && existingConfig.fonts
          ? (existingConfig.fonts as Record<string, unknown>)
          : {}),
    ...
    if (freshTenant) {
      ...
      const metaTitleToSet =
        typeof configRow.meta_title === "string" &&
        configRow.meta_title.trim().length > 0
          ? undefined
          : seoDefaults.meta_title;
      const metaDescriptionToSet =
        typeof configRow.meta_description === "string" &&
        configRow.meta_description.trim().length > 0
          ? undefined
          : seoDefaults.meta_description;
      const ogImageUrlToSet =
        typeof configRow.og_image_url === "string" &&
        configRow.og_image_url.trim().length > 0
          ? undefined
