...
export interface AdminDomainRequest {
  ...
  fullDomain:      string
  priority:        number
  status:          string             // waas_domain_status
...
export async function getDomainRequests(opts: {
  ...
  try {
    ...
    let q = supabase
      .from('domain_requests')
      .select(`
        id, tenant_id, domain_name, extension, full_domain, priority,
        status, workflow_status, notes, admin_notes, status_history,
        actioned_at, actioned_by, created_at, updated_at
      `)
      .order('created_at', { ascending: false })

    if (workflowStatus !== 'all') {
      q = q.eq('workflow_status', workflowStatus)
    }
    ...
    // Enrich with business name from tenants
...
export async function updateDomainRequestStatus(
  ...
  try {
    ...
    // Fetch current row to append to history
    ...
    if (fetchErr || !currentRow) {
      return { success: false, error: 'Domain request not found.' }
    }

    const row = currentRow as {
      workflow_status: string
      status_history:  DomainStatusHistoryEntry[]
      admin_notes:     string | null
    }

    const historyEntry: DomainStatusHistoryEntry = {
      workflowStatus: row.workflow_status as DomainWorkflowStatus,
      changedBy,
      changedAt: new Date().toISOString(),
    }

    const newHistory: DomainStatusHistoryEntry[] = [
