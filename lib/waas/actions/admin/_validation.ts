...
export function validateVariantSections(
  sections: SectionConfig[],
): string | null {
  for (const section of sections) {
    ...
    const headlineErr = validateStringLength(
      headline,
      140,
      `${section.section} headline`,
    );
    if (headlineErr) return headlineErr;
    const subheadlineErr = validateStringLength(
      subheadline,
      700,
      `${section.section} subheadline`,
    );
    if (subheadlineErr) return subheadlineErr;
    const eyebrowErr = validateStringLength(
      eyebrow,
      60,
    ...
    if (section.section === "about") {
      ...
      if (Array.isArray(highlights)) {
        if (highlights.length > 10)
          return "About highlights are limited to 10 items.";
        for (const item of highlights) {
          if (typeof item !== "string")
            return "About highlights must be text items.";
          if (item.length > 120)
            return "Each About highlight must be 120 characters or fewer.";
        }
      }
    }
      ...
      if (Array.isArray(faqItems)) {
        ...
        for (const item of faqItems) {
          if (!item || typeof item !== "object")
            return "FAQ items must be objects.";
          const row = item as Record<string, unknown>;
          const question =
          ...
          if (question.length > 180)
            return "FAQ questions must be 180 characters or fewer.";
          if (answer.length > 700)
            return "FAQ answers must be 700 characters or fewer.";
        }
      }
    }
    if (section.section === "how-it-works") {
      ...
      if (Array.isArray(steps)) {
        ...
        for (const item of steps) {
          if (!item || typeof item !== "object")
            return "How It Works steps must be objects.";
          const row = item as Record<string, unknown>;
          ...
          if (!description)
            return "Each How It Works step requires a description.";
          if (title.length > 100)
            return "How It Works step titles must be 100 characters or fewer.";
          if (description.length > 320)
            return "How It Works step descriptions must be 320 characters or fewer.";
        }
      }
    }
    if (section.section === "services") {
      ...
      if (Array.isArray(items)) {
        ...
        for (const item of items) {
          if (!item || typeof item !== "object")
            return "Service items must be objects.";
          const row = item as Record<string, unknown>;
          const title = typeof row.title === "string" ? row.title.trim() : "";
          if (!title) return "Each service item requires a title.";
          if (title.length > 90)
            return "Service item titles must be 90 characters or fewer.";
          const description =
            typeof row.description === "string" ? row.description : null;
          const descriptionErr = validateStringLength(
            description,
            260,
...
export function getVariantCoreSectionFailures(
  sections: SectionConfig[],
): string[] {
...
export function validateVariantReviewReadiness(
  variantIndex: number,
  sections: SectionConfig[],
): string | null {
