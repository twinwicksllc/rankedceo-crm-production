...
export interface TenantDetailData {
  ...
  siteConfig:
    (TenantSiteConfig & { site_templates?: { slug: string } | null }) | null;
...
export async function getAdminTenants(): Promise<
  ActionResult<AdminTenantListItem[]>
> {
  try {
    ...
    if (error && parseMissingTenantColumn(error.message) === "deleted_at") {
      ...
      if (error && isPendingReviewEnumError(error.message)) {
        const retryStatuses = statuses.filter(
    ...
    const { data: siteConfigRows } = await supabase
      ...
      .select(
        "tenant_id, client_review_token, client_selected_template_slug, client_selected_at",
      )
    ...
    for (const row of (siteConfigRows ?? []) as Array<
    ...
    >) {
      ...
      siteConfigMap.set(tenantId, {
        client_review_token:
          (row.client_review_token as string | null | undefined) ?? null,
        client_selected_template_slug:
          (row.client_selected_template_slug as string | null | undefined) ??
          null,
        client_selected_at:
          (row.client_selected_at as string | null | undefined) ?? null,
      });
    }
...
export async function getRecentlyArchivedTenants(
  limit = 8,
): Promise<ActionResult<ArchivedTenantListItem[]>> {
  try {
    const supabase = getAdminClient();
    ...
    const { data, error } = await supabase
      .from("tenants")
      .select(
        "id, legal_name, brand_config, submitted_by_email, deleted_at, created_at",
      )
  ...
  try {
    ...
    if (error) {
      if (parseMissingTenantColumn(error.message) === "deleted_at") {
        return {
          success: false,
          error: "This environment does not support archived tenant restore.",
        };
      }
      return { success: false, error: error.message };
    }
...
export async function archiveDuplicatePendingAttempts(): Promise<
...
> {
  try {
    ...
    let query = supabase
      .from("tenants")
      .select(
        "id, created_at, submitted_by_email, legal_name, brand_config, status",
      )
      .in("status", statuses)
      .is("deleted_at", null)
      .order("created_at", { ascending: false });
    ...
    for (const row of rows) {
      const brand =
        (row.brand_config as Record<string, unknown> | null | undefined) ?? {};
      const businessName =
...
export async function getTenantDetail(
  tenantId: string,
): Promise<ActionResult<TenantDetailData>> {
  try {
    const supabase = getAdminClient();
    ...
    if (tenantRow.source_audit_id) {
      const { data: auditData } = await supabase
        .from("audits")
        .select(
          "id, status, report_data, target_url, competitor_urls, completed_at",
        )
    ...
    if (resolvedDomainRequests.length === 0) {
      const brandConfigRecord = tenantRow.brand_config as unknown as
        Record<string, unknown> | undefined;
      const domainWishlist = brandConfigRecord?.domain_wishlist;
      if (Array.isArray(domainWishlist)) {
        resolvedDomainRequests = domainWishlist
          .map((item, index) => {
            const row = (item ?? {}) as Record<string, unknown>;
            const domainName =
              typeof row.domain_name === "string" ? row.domain_name : "";
            const extension =
              typeof row.extension === "string" ? row.extension : ".com";
            const normalizedStatus =
              typeof row.status === "string" ? row.status : "requested";
            return {
    ...
    const { data: deploymentsData, error: deploymentsError } = await supabase
      ...
      .select(
        "id, deployed_by, source_version_id, deployment_payload_json, created_at",
      )
      .eq("tenant_id", tenantId)
      .order("created_at", { ascending: false })
      .limit(10);
    if (!deploymentsError) {
      deploymentsRows = (deploymentsData ?? []) as Array<
        Record<string, unknown>
      >;
    }
      ...
      data: {
        ...
        deployments: deploymentsRows.map((row) => ({
          ...
          deployment_payload_json:
            row.deployment_payload_json &&
            typeof row.deployment_payload_json === "object"
...
export async function searchTenants(
  ...
  try {
    ...
    let q = supabase
      ...
      .select("*")
      .is("deleted_at", null)
      .order(sortBy === "business_name" ? "slug" : "created_at", {
    ...
    if (query.trim()) {
      ...
      tenants = tenants.filter((t) => {
        ...
        return (
          name.includes(q_lower) ||
          slug.includes(q_lower) ||
          domain.includes(q_lower)
        );
      });
    }
...
export async function bulkUpdateTenantStatus(
  ...
  const newStatus: WaasTenantStatus =
