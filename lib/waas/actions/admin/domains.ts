...
export async function getDomainRequests(
  ...
  try {
    ...
    let q = supabase
      .from("domain_requests")
      .select(
        `id, tenant_id, domain_name, extension, full_domain, priority, status, workflow_status, notes, admin_notes, status_history, actioned_at, actioned_by, created_at, updated_at`,
      )
    ...
    for (const tr of (tenantRows ?? []) as Array<{
      id: string;
    ...
    return {
      ...
      data: rows.map((r) => ({
        ...
        workflowStatus: (r.workflow_status ??
          "requested") as DomainWorkflowStatus,
