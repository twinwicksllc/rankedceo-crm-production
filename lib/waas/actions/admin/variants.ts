...
import {
  ...
  VARIANT_LIFECYCLE_SOURCES,
  normalizeReasonCategory,
  getDefaultReasonCategoryForSource,
} from "./_versioning";
import type { VariantLifecycleReasonCategory } from "./_versioning";
import {
  toSectionConfigList,
  normalizeVariantSections,
  validateVariantSections,
  getVariantCoreSectionFailures,
  validateVariantReviewReadiness,
} from "./_validation";
...
function mapSiteVariantRow(row: Record<string, unknown>): AdminSiteVariant {
  return {
    ...
    variant_label:
      typeof row.variant_label === "string" ? row.variant_label : "Variant",
    variant_rationale:
      typeof row.variant_rationale === "string" ? row.variant_rationale : null,
    template_slug:
      typeof row.template_slug === "string" ? row.template_slug : "modern",
    sections_json: Array.isArray(row.sections_json)
      ? (row.sections_json as SectionConfig[])
      : [],
    generation_notes:
      typeof row.generation_notes === "string" ? row.generation_notes : null,
...
export async function getTenantVariantStatuses(
  tenantId: string,
): Promise<string[]> {
  ...
  return ((data ?? []) as Array<Record<string, unknown>>).map((row) =>
    typeof row.status === "string" ? row.status : "generated",
  );
}

export async function saveVariantHistorySnapshot(
  tenantId: string,
  variantIndex: number,
  summary: string,
): Promise<void> {
  try {
    ...
    const { data: variantRow, error: variantError } = await supabase
      .from("tenant_site_variants")
      .select(
        "variant_index, variant_label, variant_rationale, template_slug, sections_json",
      )
      .eq("tenant_id", tenantId)
...
export async function getSiteVariants(
  tenantId: string,
): Promise<ActionResult<AdminSiteVariant[]>> {
  try {
    const supabase = getAdminClient();
    const { data, error } = await supabase
      .from("tenant_site_variants")
      .select(
        "id, variant_index, variant_label, variant_rationale, template_slug, sections_json, generation_notes, status, generated_at",
      )
      .eq("tenant_id", tenantId)
  ...
  try {
    ...
    if (variantsError) {
      if (isMissingSchemaTable(variantsError.message, "tenant_site_variants"))
        return {
          success: false,
          error: "Site variants are not available in this environment.",
        };
    ...
    if (variantRows.length < 3)
      reportIssues.push(
        "Cannot send to client review until 3 variants are generated.",
      );
    for (const row of variantRows) {
      const variantIndex =
        typeof row.variant_index === "number" ? row.variant_index : 0;
      const sectionsRaw = Array.isArray(row.sections_json)
        ? row.sections_json
        : [];
      const sections = normalizeVariantSections(
        toSectionConfigList(sectionsRaw),
      );
      ...
      if (sections.length === 0) {
        ...
        const readinessError = validateVariantReviewReadiness(
          variantIndex,
          sections,
        );
        if (readinessError)
          issues.push(
            readinessError.replace(
              new RegExp(`^Variant ${variantIndex}:\\s*`),
              "",
            ),
          );
      }
...
export async function getVariantLifecycleTelemetry(
  ...
  try {
    ...
    if (!variantError) {
      variantStatuses = (
        (variantRows ?? []) as Array<Record<string, unknown>>
      ).map((row) => ({
        variantIndex:
          typeof row.variant_index === "number" ? row.variant_index : 0,
        variantLabel:
      ...
      !isMissingSchemaTable(variantError.message, "tenant_site_variants")
    ) {
    ...
    if (configRow && typeof configRow === "object") {
      ...
      selectedTemplateSlug =
        typeof row.client_selected_template_slug === "string"
          ? row.client_selected_template_slug
          : null;
      selectedAt =
        typeof row.client_selected_at === "string"
    ...
    const { data: versionRows, error: versionError } = await supabase
      .from("tenant_site_versions")
      .select(
        "id, change_source, summary, template_slug, created_at, snapshot_json",
      )
      .eq("tenant_id", tenantId)
      ...
      .limit(20);
    if (!versionError) {
      events = ((versionRows ?? []) as Array<Record<string, unknown>>).map(
        (row) => ({
          ...(function parseMeta() {
            const snapshot =
              row.snapshot_json && typeof row.snapshot_json === "object"
                ...
                : null;
            const meta =
              snapshot?.lifecycle_event_meta &&
              typeof snapshot.lifecycle_event_meta === "object"
            ...
            return {
              reasonCategory,
              reasonText:
                typeof meta?.reasonText === "string" ? meta.reasonText : null,
              actorType: (typeof meta?.actorType === "string"
                ? meta.actorType
                : String(row.change_source ?? "").startsWith("client_")
                  ? "public_client"
                  : "system") as VariantLifecycleEvent["actorType"],
              operatorId:
                typeof meta?.operatorId === "string" ? meta.operatorId : null,
              operatorEmail:
                typeof meta?.operatorEmail === "string"
                  ? meta.operatorEmail
                  : null,
              operatorRole:
                typeof meta?.operatorRole === "string"
                  ? meta.operatorRole
                  : null,
            };
          })(),
          id: typeof row.id === "string" ? row.id : "",
          changeSource:
            typeof row.change_source === "string"
              ? row.change_source
              : "unknown_change",
      ...
      !isMissingSchemaTable(versionError.message, "tenant_site_versions")
    ) {
      return { success: false, error: versionError.message };
    }
    const reviewState: VariantLifecycleTelemetry["reviewState"] =
      variantStatuses.some((v) => v.status === "selected")
        ? "selected"
        : variantStatuses.some((v) => v.status === "sent_to_review")
          ? "in_review"
          : "editing";
    const lastReviewSentAt =
      events.find((e) => e.changeSource === "site_variants_sent_to_review")
        ?.createdAt ?? null;
    const lastUnlockedAt =
      events.find(
        (e) => e.changeSource === "site_variants_unlocked_for_editing",
      )?.createdAt ?? null;
    return {
      success: true,
      data: {
...
export async function updateSiteVariant(
  ...
  try {
    if (!Number.isInteger(variantIndex) || variantIndex < 1 || variantIndex > 3)
      return {
        success: false,
        error: "Variant index must be between 1 and 3.",
      };
    const supabase = getAdminClient();
    ...
    if (statuses.includes("sent_to_review") || statuses.includes("selected"))
      return {
        success: false,
        error:
          "Variant editing is locked while client review is active. Unlock variants before editing.",
      };
    ...
    if (Array.isArray(input.sections)) {
      const safeSections = normalizeVariantSections(
        toSectionConfigList(input.sections),
      );
    ...
    if (error) {
      if (isMissingSchemaTable(error.message, "tenant_site_variants"))
        return {
          success: false,
          error: "Site variant storage is not available in this environment.",
        };
      return { success: false, error: error.message };
    }
...
export async function getVariantEditHistory(
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .eq("tenant_id", tenantId)
      .in("change_source", [
        "variant_edit_snapshot",
        "variant_rollback_applied",
      ])
      .order("created_at", { ascending: false })
    ...
    for (const row of (data ?? []) as Array<Record<string, unknown>>) {
      const snapshot =
        row.snapshot_json && typeof row.snapshot_json === "object"
          ? (row.snapshot_json as Record<string, unknown>)
          : null;
      const snapshotVariantIndex =
        typeof snapshot?.variant_index === "number"
  ...
  try {
    ...
    if (statuses.includes("sent_to_review") || statuses.includes("selected"))
      return {
        success: false,
        error:
          "Variant editing is locked while client review is active. Unlock variants before rollback.",
      };
    const supabase = getAdminClient();
    ...
    if (versionError || !versionRow)
      return {
        success: false,
        error: versionError?.message ?? "Snapshot not found.",
      };
    const snapshot =
      (versionRow as { snapshot_json?: Record<string, unknown> | null })
        .snapshot_json ?? null;
    const snapshotVariantIndex =
      typeof snapshot?.variant_index === "number"
        ? snapshot.variant_index
    ...
    if (!safeSections || safeSections.length === 0)
      return {
        success: false,
        error: "Snapshot is missing valid section data.",
      };
    const validationError = validateVariantSections(safeSections);
    if (validationError)
      return {
        success: false,
        error: `Snapshot failed validation: ${validationError}`,
      };
    await saveVariantHistorySnapshot(
      tenantId,
      variantIndex,
    ...
    if (
      typeof snapshot?.variant_label === "string" &&
      snapshot.variant_label.trim()
    )
      patch.variant_label = snapshot.variant_label.trim().slice(0, 120);
    if (typeof snapshot?.variant_rationale === "string") {
      const n = snapshot.variant_rationale.trim();
      patch.variant_rationale = n ? n.slice(0, 500) : null;
    }
...
function setSectionConfig(
  ...
  return sections.map((section) => {
    ...
    return {
      ...section,
      ...changes,
      config: { ...section.config, ...(changes.config ?? {}) },
    };
  });
}
...
export async function reorderVariantSections(
  ...
  try {
    ...
    for (const id of orderedIds) {
      if (seen.has(id))
        return { success: false, error: `Duplicate section id: ${id}` };
      seen.add(id);
    }
    ...
    if (!variantsResult.success || !variantsResult.data)
      return {
        success: false,
        error: variantsResult.error ?? "Failed to load variants",
      };
    const variant = variantsResult.data.find(
      (v) => v.variant_index === variantIndex,
    );
    if (!variant)
      return {
        success: false,
        error: `Variant ${variantIndex} not found for tenant ${tenantId}`,
      };
    ...
    for (const id of orderedIds) {
      if (!sectionMap.has(id))
        return {
          success: false,
          error: `Section "${id}" does not exist in variant ${variantIndex}`,
        };
    }
