...
export async function generateTemplateRecommendations(
  ...
  try {
    ...
    const { data: tenant, error } = await supabase
      ...
      .eq('id', tenantId)
      .single()
    if (error || !tenant) return { success: false, error: error?.message ?? 'Tenant not found' }
...
export async function getAdminStats(): Promise<ActionResult<AdminStats>> {
  try {
    ...
    const countTenants = async (statuses: string[]) => {
      ...
      let result = await supabase.from('tenants').select('id', { count: 'exact', head: true }).in('status', queryStatuses).is('deleted_at', null)
      if (result.error && isPendingReviewEnumError(result.error.message)) {
        queryStatuses = queryStatuses.filter((s) => s !== 'pending_review')
        result = await supabase.from('tenants').select('id', { count: 'exact', head: true }).in('status', queryStatuses).is('deleted_at', null)
      }
      if (result.error && parseMissingTenantColumn(result.error.message) === 'deleted_at') {
        result = await supabase.from('tenants').select('id', { count: 'exact', head: true }).in('status', queryStatuses)
        if (result.error && isPendingReviewEnumError(result.error.message)) {
          queryStatuses = queryStatuses.filter((s) => s !== 'pending_review')
          result = await supabase.from('tenants').select('id', { count: 'exact', head: true }).in('status', queryStatuses)
        }
    ...
    return { success: true, data: { pendingCount, activeCount, totalLeads: leadsRes.count ?? 0 } }
  } catch (err) {
    return { success: false, error: err instanceof Error ? err.message : 'Unknown error' }
  }
}
...
export async function getWaasRevenueStats(): Promise<ActionResult<WaasRevenueStats>> {
  try {
    ...
    const rows = (data ?? []) as Array<{ id: string; brand_config: Record<string, unknown> | null; package_tier: string | null; plan_interval: string | null; stripe_subscription_id: string; created_at: string }>
    const MONTHLY_VALUE: Record<string, number> = { hosting_only: Math.round(199 / 12), standard: 39, premium: 49 }
    const YEARLY_VALUE:  Record<string, number> = { hosting_only: 199, standard: 399, premium: 499 }
    let mrr = 0, arr = 0
    ...
    for (const row of rows) {
      ...
      if (interval === 'month') { mrr += MONTHLY_VALUE[tier] ?? 0; arr += (MONTHLY_VALUE[tier] ?? 0) * 12 }
      else if (interval === 'year') { const y = YEARLY_VALUE[tier] ?? 0; mrr += Math.round(y / 12); arr += y }
    }
    const recentSubscriptions = rows.slice(0, 10).map((row) => {
      const bc = row.brand_config as { business_name?: string } | null
      return { tenantId: row.id, businessName: bc?.business_name ?? row.id.slice(0, 8), packageTier: row.package_tier ?? 'hosting', planInterval: row.plan_interval, createdAt: row.created_at }
    })
    return { success: true, data: { mrr, arr, activePaidCount: rows.length, planBreakdown, recentSubscriptions } }
  } catch (err) {
