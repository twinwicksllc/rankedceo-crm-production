...
import {
  getTenantVariantStatuses,
  getVariantReviewReadiness,
  saveVariantHistorySnapshot,
} from "./variants";
  ...
  try {
    ...
    if (statuses.includes("selected")) {
      return {
        success: false,
        error:
          "Cannot unlock because a client selection already exists. Regenerate variants or clear selection first.",
      };
    }
...
export async function reopenVariantReviewCycle(
  ...
  try {
    ...
    if (!normalizedReason || normalizedReason.length < 10)
      return {
        success: false,
        error: "Provide a reopen reason of at least 10 characters.",
      };
    const statuses = await getTenantVariantStatuses(tenantId);
    if (!statuses.includes("selected")) {
      return {
        success: false,
        error:
          "No selected variant exists. Use unlock when review is active without a final selection.",
      };
    }
    ...
    if (
      configError &&
      !isMissingSchemaTable(configError.message, "tenant_site_config")
    )
...
export async function markVariantsSentToReview(
  tenantId: string,
  ...
  try {
    ...
    if (statuses.includes("selected")) {
      return {
        success: false,
        error:
          "Client selection already exists. Regenerate variants before starting a new review cycle.",
      };
    }
    if (
      statuses.includes("sent_to_review") &&
      !statuses.includes("generated")
    ) {
      return {
        success: false,
        error:
          "Variants are already in client review. Unlock variants before sending again.",
      };
    }
    ...
    if (!readiness.success || !readiness.data)
      return {
        success: false,
        error:
          readiness.error ?? "Unable to validate variant review readiness.",
      };
    if (!readiness.data.ready)
      return {
        success: false,
        error:
          readiness.data.issues[0] ??
