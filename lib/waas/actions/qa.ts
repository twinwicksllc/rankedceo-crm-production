...
export async function getQaRunDetail(
  runId: string,
...
export async function purgeQaRuns(): Promise<{
  purged: number;
  error?: string;
}> {
  try {
...
export async function listQaScenarios(): Promise<{
  data: QaScenario[] | null;
  error?: string;
}> {
  try {
    const supabase = getWaasQaClient();
...
export async function updateQaScenario(
  id: string,
  input: {
    name?: string;
    description?: string;
...
export async function deleteQaScenario(
  id: string,
