...
function classifyEditType(path: string, _value: JsonValue): EditType {
...
export async function getClientEditHistory(
  ...
  try {
    ...
    // Filter to a specific variant if requested
    if (variantIndex !== undefined) {
      // Join via variant_id would require a separate query; here we filter post-fetch
      // or rely on a denormalized variant_index column if we add one in a future migration.
      // For now, return all events for the tenant (scoped to the token).
    }
