...
//   regenerateSection             — PR #102: regenerate all text fields in a
//                                   single section using AI, with an optional
//                                   client hint, returning the updated fields
//                                   map so the editor can apply them in bulk.
// =============================================================================
...
export async function submitDomainChangeRequest(
  ...
  try {
    ...
    return {
      success: false,
      error: err instanceof Error ? err.message : 'Failed to submit domain request',
    }
  }
}
...
export async function getTenantAuditHistory(
  ...
  try {
    ...
    const items: TenantAuditHistoryItem[] = ((data ?? []) as Array<{
      id:          string
      status:      string
      target_url:  string
    ...
    return { success: false, error: err instanceof Error ? err.message : 'Failed to load audit history' }
  }
}
...
const REGENERATABLE_LEAF_FIELDS: Partial<Record<SectionId, Array<{
  key:      string
  label:    string
  maxLen:   number
  isArray?: false
...
export async function regenerateSection(
  ...
  if (session.permissions.isLocked) {
    return { success: false, error: 'Editing is locked — approval has been submitted.' }
  }
  ...
  if (!leafFields || leafFields.length === 0) {
    return { success: false, error: `Section "${sectionId}" does not support AI regeneration.` }
  }

  const openAiKey = process.env.OPENAI_API_KEY
  if (!openAiKey) return { success: false, error: 'AI regeneration is not configured.' }
  ...
  const variantRows = variantRow as Array<{ sections_json: unknown; id: string }>
  const targetRow   = variantRows[variantIndex]
  if (!targetRow) {
    return { success: false, error: `Variant at index ${variantIndex} not found.` }
  }
  ...
  if (!section) {
    return { success: false, error: `Section "${sectionId}" not found in this variant.` }
  }
  ...
  const businessName = typeof brand.business_name === 'string' && brand.business_name
    ? brand.business_name
    : (tenantRow.legal_name ?? 'Your Business')
  const trade    = tenantRow.primary_trade ?? tenantRow.target_industry ?? 'Local service'
  const location = [contactInfo.city, contactInfo.state].filter(Boolean).join(', ') || 'your area'
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return { success: false, error: `AI service error: ${resp.status} — ${errText.slice(0, 200)}` }
    }
    ...
    return {
      success: false,
      error: err instanceof Error ? err.message : 'Unexpected AI error. Please try again.',
    }
  }

  // Must have produced at least one field
  if (Object.keys(rawResponse).length === 0) {
    return { success: false, error: 'AI returned no usable content. Please try again.' }
  }
  ...
  const fields: RegeneratedField[] = leafFields
    ...
    .map((lf) => ({
      ...
      label:     lf.label,
      original:  existingContent[lf.key] != null ? String(existingContent[lf.key]) : '',
