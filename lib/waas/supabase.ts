...
export async function resolveTenantByHostname(
  ...
  try {
    ...
    try {
      ...
      console.log("[WaaS] Supabase client created successfully");
    } catch (clientErr) {
      console.error("[WaaS] Failed to create Supabase client:", {
        error:
    ...
    const { data, error } = await client.rpc("resolve_tenant_by_hostname", {
      p_hostname: hostname,
    });
...
export async function getAuditStatus(
  auditId: string,
): Promise<AuditStatusResult | null> {
  try {
    const client = getRawAdminClient();
    const { data, error } = await client.rpc("get_audit_status", {
      p_audit_id: auditId,
    });
