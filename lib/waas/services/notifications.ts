...
async function sendViaResend(opts: {
  ...
  try {
    ...
    return { messageId: null, error: err instanceof Error ? err.message : 'Resend fetch failed' }
  }
}
...
async function sendViaSendGrid(opts: {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      return { messageId: null, error: `HTTP ${resp.status}: ${text.slice(0, 200)}` }
    }

    const messageId = resp.headers.get('X-Message-Id')
    return { messageId }
  } catch (err) {
    return { messageId: null, error: err instanceof Error ? err.message : 'SendGrid fetch failed' }
  }
}
...
async function isDuplicate(
  ...
  try {
    ...
    const windowAgo  = new Date(Date.now() - windowHours * 60 * 60 * 1000).toISOString()
...
async function resolveTenantEmail(tenantId: string, override?: string): Promise<string | null> {
...
export async function sendTenantNotification(
  ...
  if (!recipientEmail) {
    console.warn(`[notifications] No email for tenant ${tenantId} — skipping ${type}`)
    return { sent: false, skipped: true, provider: 'none', messageId: null, error: 'No recipient email' }
  }
  ...
  // 5. Log result
  await logNotification({
    tenantId, recipientEmail, type, subject,
    templateData:      data,
    status,
    provider,
