...
export function generateMetaTitle(tenant: WaasTenant): string {
  const businessName =
    tenant.legal_name || tenant.brand_config?.business_name || "Business";
...
export function generateMetaDescription(tenant: WaasTenant): string {
  const businessName =
    tenant.legal_name || tenant.brand_config?.business_name || "Our business";
  const primaryTrade = tenant.primary_trade || "services";
  ...
  const locationSuffix =
    city && state ? ` in ${city}, ${state}` : city ? ` in ${city}` : "";
