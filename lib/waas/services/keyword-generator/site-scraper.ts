...
export async function fetchHtml(url: string): Promise<string | null> {
  try {
    const response = await fetch(url, {
      ...
      headers: {
        "User-Agent":
          "Mozilla/5.0 (compatible; RankedCEOAuditBot/1.0; +https://rankedceo.com)",
      },
    });
...
export async function collectSiteSignals(
  ...
  const usefulPaths = links
    .filter((link) => {
      ...
      return /(about|contact|service|services|location|areas|company)/.test(
        pathname,
      );
    })
    .slice(0, 2);
  ...
  const mergedText = cleanText(
    [
      ...
      ...extraPages.flatMap((page) =>
        page ? [page.title ?? "", page.description ?? "", page.text] : [],
      ),
    ].join(" "),
  ...
  const locationHint = addressHint
    ? (addressHint.match(
        /,\s*([A-Za-z .'\-]{2,40},\s*[A-Z]{2})\s*\d{5}(?:-\d{4})?\b/,
      )?.[1] ?? null)
  ...
  return {
    ...
    titleHints: [
      title,
      metaDescription,
      ...extraPages.map((page) => page?.title ?? null),
