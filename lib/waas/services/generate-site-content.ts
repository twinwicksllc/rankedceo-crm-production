  ...
  return {
    ...
    location:
      ...
      ([tenant.city, tenant.state].filter(Boolean).join(", ") ||
        "your local area"),
    usp:
      typeof tenant.usp === "string" && tenant.usp.trim()
        ? tenant.usp.trim()
    ...
    tagline:
      typeof brand.tagline === "string" && brand.tagline.trim()
        ...
        : "",
    primaryCta:
      typeof content.primary_cta === "string" && content.primary_cta.trim()
        ? content.primary_cta.trim()
        : "Book a Free Estimate",
    aboutNarrative:
      typeof content.about_narrative === "string" &&
    ...
    targetAudience:
      typeof intake.target_audience === "string"
    ...
    tone:
      typeof brand.tone === "string" && brand.tone.trim()
        ...
        : "Professional",
    serviceArea:
      typeof seo.service_area === "string" && seo.service_area.trim()
...
function buildFaqContent(profile: GenerationProfile): FAQSectionContent {
  ...
  const items = [
    ...
    },
    {
      ...
      answer:
        "We confirm availability quickly and align scheduling based on urgency and location.",
    },
    {
...
function buildProcessContent(
  ...
  return {
    ...
    steps: [
      {
        title: "Reach Out",
        description: `Tell us about your project and goals in ${profile.location}.`,
      },
      {
        title: "Plan & Quote",
        description: "We review options, timelines, and transparent pricing.",
      },
      {
        title: "Deliver",
        description:
...
function buildDeterministicVariants(
  profile: GenerationProfile,
  ...
  const services =
    profile.services.length > 0
      ? profile.services
          ...
          .map((service) => ({
            title: service,
            description: `${service} handled by trained professionals.`,
          }))
      : undefined;
  ...
  const commonPatches: Array<
    Partial<GeneratedSiteVariant> & {
      templateSlug: string;
      variantIndex: number;
      variantLabel: string;
  ...
  > = [
    {
      ...
      variantRationale:
        "Leads with USP and direct calls-to-action for high-intent visitors.",
      templateSlug: "bold",
    },
    {
      variantIndex: 2,
      ...
      variantRationale:
        "Highlights credibility and narrative to build trust quickly.",
    ...
    },
    {
      ...
      variantRationale:
        "Focuses on expertise, process clarity, and FAQ confidence builders.",
      templateSlug: "modern",
    },
  ];
  ...
  return commonPatches.map((base) => {
    let sections = cloneSections(
      getTemplate(base.templateSlug).default_layout_json,
    );
  ...
  return value
    ...
    .filter(
      (row) =>
        ...
        allowed.includes(row.section as SectionId),
    )
    .map((row, index) => ({
...
function sanitizeVariants(
  ...
  for (const item of raw) {
    ...
    const variantIndex =
      typeof row.variantIndex === "number" ? row.variantIndex : out.length + 1;
    const templateSlug =
      typeof row.templateSlug === "string" && row.templateSlug.trim()
        ? row.templateSlug.trim()
        : "modern";
    ...
    out.push({
      ...
      variantRationale:
        typeof row.variantRationale === "string" && row.variantRationale.trim()
...
async function generateWithGemini(
  profile: GenerationProfile,
  notes?: string,
): Promise<GeneratedSiteVariant[] | null> {
...
export async function generateSiteVariants(
  tenant: WaasTenant,
  notes?: string,
): Promise<GeneratedSiteVariant[]> {
