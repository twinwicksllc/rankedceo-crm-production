...
import { INDUSTRY_TEMPLATE_MAP, DEFAULT_INDUSTRY_TEMPLATES, getRecommendedTemplateSlugs } from '@/lib/waas/templates/registry'
...
function fallbackRecommendations(
  ...
  const scoreTemplate = (t: SiteTemplate): number => {
    ...
    if (tenant.financingEnabled && ['bold', 'conversion', 'consultative', 'emergency'].includes(t.slug)) {
      score += 4
    }

    // Boost by booking signal
    if (tenant.hasBooking && ['conversion', 'emergency', 'modern', 'local-pro'].includes(t.slug)) {
      score += 3
    }

    // Boost by tone
    const tone = (tenant.tone ?? '').toLowerCase()
    if (tone.includes('professional') && ['modern', 'premium', 'consultative'].includes(t.slug)) score += 3
    if (tone.includes('friendly') && ['community', 'local-pro'].includes(t.slug)) score += 3
    if (tone.includes('urgent') && ['emergency', 'bold', 'conversion'].includes(t.slug)) score += 3

    // USP length signals depth — favour consultative / trust for longer USPs
    const uspLen = (tenant.usp ?? '').length
    if (uspLen > 60 && ['consultative', 'trust-first', 'premium'].includes(t.slug)) score += 3
  ...
  return templates
    .map(t => ({
      ...
      seoRationale: seoRationaleMap[t.slug] ?? `${t.name} is optimised for ${(t as { seo_strategy?: string }).seo_strategy ?? 'local service'} SEO.`,
...
function sanitizeRecommendations(
  ...
  for (const item of raw) {
    ...
    const seoRationale = typeof rec.seoRationale === 'string' && rec.seoRationale.trim()
...
export async function recommendTemplates(
  ...
  const allowedSet = new Set([...recSlugs, ...fallback.slice(0, 5).map(r => r.templateSlug)])
