...
export function inferGlFromLocation(location: string): string {
  ...
  if (/(united states|usa|u\.s\.|\b[a-z]{2},\s*[a-z]{2}\b)/.test(value))
    return "us";
  if (/(united kingdom|uk|england|scotland|wales|northern ireland)/.test(value))
    return "uk";
...
export function stripLocationFromKeyword(
  keyword: string,
  location: string,
): string {
  const { city, state } = parseLocationParts(location);
