...
function buildGeminiPrompt(
  ...
  const strategyInstructions: Record<SeoStrategy, string> = {
    "local-service":
      'Weight "[City] [Trade]" keyword clusters throughout headlines and subheadlines. Ensure NAP-style information is present in the hero and about sections.',
    "trust-authority":
      "Emphasise credentials, certifications, years of experience, and E-E-A-T signals. Every section should reinforce expertise and trustworthiness.",
    "visual-portfolio":
      "Lead with project outcomes and visual language. Hero and services copy should invoke imagery of completed work and transformation.",
    emergency:
      'Use urgency language throughout: "fast", "24/7", "same-day". Every CTA must convey immediacy. Hero headline must include a strong availability signal.',
    consultative:
      "Use a consultative, educational tone. FAQ and How-It-Works sections must be detailed and step-oriented. Avoid salesy language.",
    conversion:
      'Maximise CTA density. Every section should include a micro-conversion hook. Use offer language ("free estimate", "no obligation") naturally.',
  };

  const toneInstructions = [
    template.mood &&
      `The template aesthetic is "${template.mood}". Tone the copy to match.`,
    profile.tone &&
      profile.tone !== "Professional" &&
      `The business prefers a "${profile.tone}" voice.`,
  ]
    .filter(Boolean)
    .join(" ");
      ...
      .update({
        ...
        variant_label:
          typeof parsed.variantLabel === "string"
            ...
            : tier1.variantLabel,
        variant_rationale:
          typeof parsed.variantRationale === "string"
            ? parsed.variantRationale
            : tier1.variantRationale,
