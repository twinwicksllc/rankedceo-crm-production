...
> = {
  "answer-first-aeo": {
    arrayKey: "items",
    itemLabel: (i) => `Q&A Card ${i + 1}`,
    leafs: [
      { key: "question", label: "Question", kind: "text", maxLength: 180 },
  ...
  "bento-emergency": {
    ...
    leafs: [
      ...
      { key: "icon", label: "Icon Label", kind: "text", maxLength: 40 },
    ],
  },
  services: {
    arrayKey: "items",
  ...
  if (arrayDef) {
    ...
    if (Array.isArray(arr)) {
      arr.forEach((item, i) => {
        ...
        // answer-first-aeo items can include keyFacts: string[]
        if (sid === "answer-first-aeo") {
          const keyFacts = itemObj["keyFacts"];
          if (Array.isArray(keyFacts)) {
            keyFacts.forEach((fact, factIndex) => {
              out.push({
                id: `sec-${order}-${arrayDef.arrayKey}-${i}-keyFacts-${factIndex}`,
                path: `sections[${order}].content.${arrayDef.arrayKey}[${i}].keyFacts[${factIndex}]`,
                label: `${arrayDef.itemLabel(i)} — Key Fact ${factIndex + 1}`,
                value: fact == null ? "" : String(fact),
                kind: "text",
                group,
                scope: "section",
                sectionId: sid,
  ...
  out.push(
    {
      ...
      path: "brand_config.colors.primary",
      label: "Primary Color",
      value: c.primary ?? "#2563EB",
      kind: "color",
      group: g,
      scope: "brand",
    },
    {
      id: "brand-color-secondary",
      path: "brand_config.colors.secondary",
      label: "Secondary Color",
      value: c.secondary ?? "#1E40AF",
      kind: "color",
      group: g,
      scope: "brand",
    },
    {
      ...
      path: "brand_config.colors.accent",
      label: "Accent Color",
      value: c.accent ?? "#F59E0B",
      kind: "color",
      group: g,
      scope: "brand",
    },
  ...
  out.push(
    {
      ...
      path: "brand_config.contact.phone",
      label: "Phone",
      value: ct.phone ?? "",
      kind: "text",
      group: cg,
    ...
    },
    {
      ...
      path: "brand_config.contact.email",
      label: "Email",
      value: ct.email ?? "",
      kind: "text",
      group: cg,
    ...
    {
      id: "brand-contact-address",
      path: "brand_config.contact.address",
      label: "Address",
      value: ct.address ?? "",
      kind: "text",
      group: cg,
...
export function buildEditableFields(
  input: BuildEditableFieldsInput,
): EditableField[] {
  const out: EditableField[] = [];

  // ---- Section visibility toggles (first group — always at the top) ----
  const allSections = [...input.sections].sort(
...
export function assetSlotFromPath(path: string): string {
  ...
  const nested = path.match(
    /^sections\[(\d+)\]\.content\.([a-z_]+)\[(\d+)\]\.([a-z_]+)$/,
  );
  if (nested) {
