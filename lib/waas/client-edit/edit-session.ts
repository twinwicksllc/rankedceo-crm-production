...
export type EditSessionResult =
  ...
  | {
      ok: false;
      reason: "not_found" | "locked" | "invalid_token" | "error";
      message: string;
    };
...
export async function resolveClientEditSession(
  ...
  try {
    ...
    if (configErr || !configRow) {
      ...
      if (isUuid) {
        ...
        if (!fallbackErr && fallbackRow) {
          ...
          if (!newToken) {
            console.warn(
              "[resolveClientEditSession] tenant_id fallback succeeded but no token stored; row needs ensureClientReviewToken re-run",
            );
          }
          ...
          const { data: tenantRowFallback, error: tenantErrFallback } =
            await supabase
              .from("tenants")
          ...
          const tenantFb = tenantRowFallback as {
            id: string;
            slug: string;
          ...
          const businessNameFb =
            typeof brandConfigFb.business_name === "string"
              ? brandConfigFb.business_name
              : "Your Business";
          ...
          const withinGraceFb = approvalAtFb
            ? Date.now() - new Date(approvalAtFb).getTime() <
              APPROVAL_GRACE_PERIOD_MS
            : false;
          ...
          return {
            ...
            session: {
              ...
              selectedTemplateSlug:
                configFb.client_selected_template_slug ?? null,
              ...
              permissions: {
                ...
                canUnaprove:
                  !!approvalAtFb && withinGraceFb && !isApprovalLockedFb,
                isLocked: isApprovalLockedFb,
              },
              editSessionStartedAt:
                configFb.client_edit_session_started_at ?? null,
      ...
      console.error(
        "[resolveClientEditSession] tenant_site_config query failed:",
        {
          hasError: !!configErr,
    ...
    if (config.client_selected_template_slug) {
      ...
      if (variantRow) {
        selectedVariantIndex = (variantRow as { variant_index: number })
          .variant_index;
      }
    }
...
export async function canClientEditVariant(
  ...
  if (permissions.isLocked) {
    return {
      allowed: false,
      reason:
        "Your design has been approved and is locked for deployment. Contact support to request changes.",
    };
  }
