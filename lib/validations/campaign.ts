...
export const campaignTypeEnum = z.enum(
  ["one-time", "drip", "automation", "ab_test"],
  {
...
export const createCampaignSchema = z
  ...
  .refine(
    ...
    {
      message:
        "At least one targeting option (contacts, companies, deals, or segments) must be provided",
    },
  );
...
export const updateCampaignSchema = z.object({
  ...
  ab_test_winner_variant: z
    .number()
    .int("Variant must be an integer")
    .min(0, "Variant must be positive")
    .optional(),
});

// Campaign Sequence Validation
export const campaignSequenceStatusEnum = z.enum(
...
export const createCampaignSequenceSchema = z.object({
  ...
  name: z
    .string()
    .min(1, "Sequence step name is required")
    .max(255, "Sequence step name must be less than 255 characters"),
  description: z
    .string()
    .max(1000, "Description must be less than 1000 characters")
    .optional(),
  ...
  delay_value: z
    .number()
    .int("Delay value must be an integer")
    .min(0, "Delay value must be positive"),
  delay_unit: z.enum(["minutes", "hours", "days", "weeks"], {
    errorMap: () => ({ message: "Invalid delay unit" }),
  }),
  delay_from: z
    .enum(["campaign_start", "previous_step", "custom_date"], {
  ...
  trigger_event: z
    .string()
    .max(255, "Trigger event must be less than 255 characters")
    .optional(),
...
export type CreateCampaignSequenceInput = z.infer<
  typeof createCampaignSequenceSchema
>;
export type UpdateCampaignSequenceInput = z.infer<
  typeof updateCampaignSequenceSchema
>;
