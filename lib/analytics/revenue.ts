...
export async function getRevenueByMonth(
  ...
  for (let i = months - 1; i >= 0; i--) {
    ...
    if (error) {
      console.error(
        "[Revenue Analytics] Error fetching revenue by month:",
        error,
      );
      continue;
    }
    ...
    const revenue =
      data?.reduce((sum, deal) => sum + (deal.value || 0), 0) || 0;
...
export async function getRevenueByUser(
  ...
  const revenueByUser = new Map<
    string,
    { userName: string; revenue: number }
  >();
  ...
  return Array.from(revenueByUser.entries()).map(
...
export async function getAverageDealSize(
  ...
  if (error) {
    console.error(
      "[Revenue Analytics] Error fetching average deal size:",
      error,
    );
    return 0;
  }
