...
} from "@/lib/waas/services/notifications";
...
async function getAuditDetails(
  ...
  try {
    ...
    const report = (data as { report_data: unknown } | null)
      ?.report_data as Record<string, unknown> | null;
    if (!report) return null;

    // Extract key details from report
    const summary = report.summary as Record<string, unknown> | null;
    const businessName = (
      report.provider_meta as Record<string, unknown> | null
    )?.keyword_detected_business as string | undefined;
    const topOpportunities = (
      report.opportunities as Array<{ type?: string }> | null
    )
      ?.slice(0, 2)
      .map((o) => o.type)
    ...
    return {
      ...
      tradeType: (report.provider_meta as Record<string, unknown> | null)
        ?.keyword_detected_industry as string | undefined,
      topOpportunities:
  ...
  try {
    ...
    try {
      ...
      if (msg.includes("does not exist")) {
        console.warn(
          "[abandonment-check] audit_abandonment_tracking table not ready, skipping",
        );
        return NextResponse.json({
          success: true,
          processed: 0,
    ...
    for (const stage of ABANDONMENT_STAGES) {
      ...
      const { data: abandoned, error: selectError } = await supabase
        ...
        .or(
          `abandonment_email_sent->${stage.type}.eq.false,abandonment_email_sent->${stage.type}.is.null`,
        );

      if (selectError) {
        console.error(
          `[abandonment-check] Query error for stage ${stage.hours}h:`,
          selectError,
        );
        continue;
      }
      ...
      for (const record of abandoned) {
        ...
        try {
          ...
          const templateData: NotificationTemplateData = {
            ...
            abandonmentStage: `stage_${stage.hours}` as unknown as
          ...
          if (result.sent) {
            ...
            if (sendResult.error) {
              console.warn(
                `[abandonment-check] Failed to mark stage ${stage.hours}h sent:`,
                sendResult.error,
              );
            }
          ...
          console.error(
            `[abandonment-check] Error processing ${auditId} at stage ${stage.hours}h:`,
            err,
          );
        }
      }
