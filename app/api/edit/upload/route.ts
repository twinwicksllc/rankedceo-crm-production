...
export async function POST(request: Request) {
  try {
    ...
    const reviewToken  = (formData.get('reviewToken')  as string | null)?.trim() ?? ''
    const assetSlot    = (formData.get('assetSlot')    as string | null)?.trim() ?? ''
    const variantIdxRaw = (formData.get('variantIndex') as string | null)?.trim() ?? ''
    const fileRaw      = formData.get('file')

    // ---- Input validation ----
    if (!reviewToken) {
      return NextResponse.json({ success: false, error: 'Missing reviewToken' }, { status: 400 })
    }
    if (!assetSlot || !isValidSlug(assetSlot)) {
      return NextResponse.json({ success: false, error: 'Invalid assetSlot' }, { status: 400 })
    }
    const variantIndex = parseInt(variantIdxRaw, 10)
    if (!Number.isInteger(variantIndex) || variantIndex < 1 || variantIndex > 3) {
      return NextResponse.json({ success: false, error: 'Invalid variantIndex (must be 1–3)' }, { status: 400 })
    }
    ...
    if (!ALLOWED_MIMES.has(fileRaw.type)) {
      return NextResponse.json(
        { success: false, error: 'Please upload a JPG, PNG, WebP, SVG, or GIF image.' },
        { status: 400 },
      )
    }
    ...
    if (!sessionResult.ok) {
      return NextResponse.json({ success: false, error: sessionResult.message }, { status: 403 })
    }
    if (sessionResult.session.permissions.isLocked) {
      return NextResponse.json(
        { success: false, error: 'Editing is locked — your design has been approved.' },
        { status: 403 },
      )
    }
    if (!sessionResult.session.permissions.canSwapImages) {
      return NextResponse.json(
        { success: false, error: 'Image editing is not available for this session.' },
        { status: 403 },
      )
    }
    ...
    const ext         = fileRaw.name.split('.').pop()?.toLowerCase()?.replace(/[^a-z0-9]/g, '') || 'bin'
    ...
    const assetId = assetErr ? null : (assetRow as { id: string } | null)?.id ?? null
