...
export async function POST(request: Request) {
  try {
    ...
    console.log(
      "[Onboarding Step] Updating to step:",
      step,
      "for user:",
      user.email,
    );
    ...
    if (!userData || !userData.account_id) {
      console.error(
        "[Onboarding Step] User not found or no account_id:",
        userData,
      );
      return NextResponse.json(
        { error: "User not found or missing account_id" },
        { status: 404 },
      );
    }
    ...
    if (verifyError) {
      ...
      console.log(
        "[Onboarding Step] Verified step is now:",
        verifyData?.onboarding_step,
      );
    }
