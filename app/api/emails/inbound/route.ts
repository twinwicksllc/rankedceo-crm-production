...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      ...
      parsedEmail.to_addresses.length === 0
    ) {
    ...
    const createdEmail = await emailService.createEmailFromParsed(
      parsedEmail,
      accountId,
    );
    ...
    // Log success
    console.log(
      `[Email Inbound] Email received from ${parsedEmail.from_address}`,
      {
