...
export async function GET(request: NextRequest) {
  ...
  if (code) {
      ...
      process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!,
      {
        cookies: {
          get(name: string) {
            ...
            if (name.includes('code-verifier') || name.includes('auth-token-code-verifier')) {
    ...
    if (exchangeError) {
      ...
      const isAuditFlow = next.includes('audit.rankedceo.com') || next.includes('/audit/')
      if (
        exchangeError.message.includes('code verifier') ||
        exchangeError.message.includes('PKCE')
      ) {
        if (isAuditFlow) {
          return NextResponse.redirect('https://audit.rankedceo.com/login?error=session_expired')
        }
      }
