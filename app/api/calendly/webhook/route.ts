...
export async function POST(request: NextRequest) {
  ...
  console.log(
    "  Signing key set :",
    signingKey ? `yes (length: ${signingKey.length})` : "NO - MISSING",
  );
  ...
  if (signingKey) {
    ...
    console.warn(
      "[Calendly Webhook] ⚠️  CALENDLY_WEBHOOK_SIGNING_KEY not set - skipping verification",
    );
  }
  ...
  try {
    ...
    switch (eventType) {
      case "invitee.created": {
        ...
        const meetingUrl =
          eventData?.scheduled_event?.location?.join_url || null;
        ...
        console.log(
          "[Calendly Webhook] Invitee URI (normalized):",
          normalizedInviteeUri,
        );
        ...
        console.log(
          "[Calendly Webhook] Looking for connection with organizer URI:",
          organizerUri,
        );
        ...
        if (organizerUri) {
          ...
          if (connection && connection.length > 0) {
            ...
            const normalizedMatch = connection.find(
              (c) => c.calendly_user_uri === normalizedUri,
            );
            if (normalizedMatch) {
              accountId = normalizedMatch.account_id;
              userId = normalizedMatch.user_id;
              console.log(
                "[Calendly Webhook] Found normalized match for account:",
                accountId,
              );
            } else {
              console.log(
                "[Calendly Webhook] Available URIs in database:",
                connection.map((c) => c.calendly_user_uri),
              );
            }
          }
        }
        ...
        if (!accountId) {
          ...
          if (fallback) {
            ...
            console.log(
              "[Calendly Webhook] Using fallback connection for account:",
              accountId,
            );
          }
        }
        ...
        if (!accountId) {
          ...
          if (defaultAccountId) {
            ...
            console.log(
              "[Calendly Webhook] Using DEFAULT_ACCOUNT_ID from environment:",
              accountId,
            );
          } else {
            console.error(
              "[Calendly Webhook] No Calendly connection found and DEFAULT_ACCOUNT_ID not set",
            );
            console.error(
              "[Calendly Webhook] Appointment will NOT be created - please configure DEFAULT_ACCOUNT_ID in Vercel",
            );
            break;
          }
        }
        ...
        if (!existing) {
          ...
          if (insertError) {
            console.error(
              "[Calendly Webhook] Failed to insert appointment:",
              insertError,
            );
          } else {
            console.log(
              "[Calendly Webhook] ✅ Appointment created for:",
              inviteeEmail,
            );
          }
        } else {
          console.log(
            "[Calendly Webhook] Appointment already exists for invitee:",
            inviteeUri,
          );
        }
        break;
      ...
      case "invitee.canceled": {
        ...
        console.log(
          "[Calendly Webhook] Cancelling appointment for invitee:",
          inviteeUri,
        );
        console.log(
          "[Calendly Webhook] Invitee URI (normalized):",
        ...
        if (!existingAppointment) {
          console.warn(
            "[Calendly Webhook] ⚠️ No appointment found with calendly_invitee_uri:",
            normalizedInviteeUri,
          );
          break;
        }
        ...
        if (error) {
          console.error(
            "[Calendly Webhook] ❌ Failed to cancel appointment:",
            error,
          );
        } else {
          console.log(
            "[Calendly Webhook] ✅ Appointment cancelled successfully for invitee:",
            normalizedInviteeUri,
          );
        }
        break;
      ...
      case "invitee_no_show.created": {
        ...
        await supabase
          ...
          .ilike("calendly_invitee_uri", `%${normalizedInviteeUri}%`);
        console.log(
          "[Calendly Webhook] ✅ No-show recorded for invitee:",
          normalizedInviteeUri,
        );
        break;
      }
