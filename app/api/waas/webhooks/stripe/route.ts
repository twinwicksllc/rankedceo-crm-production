...
function planPriceString(tier: WaasPackageTier | null, interval: 'month' | 'year' | null): string {
...
interface TenantRow {
  id:           string
  brand_config: { business_name?: string; contact?: { email?: string | null } } | null
  package_tier: string
  plan_interval: string | null
}

async function getTenantByCustomerId(customerId: string): Promise<TenantRow | null> {
...
function notifyAsync(fn: () => Promise<void>): void {
  fn().catch((err) => {
    console.error('[WaaS Webhook] Notification error:', err instanceof Error ? err.message : String(err))
  })
}
  ...
  try {
    switch (event.type) {
      ...
      case 'checkout.session.completed': {
        ...
        const tier           = (session.metadata?.package_tier ?? null) as WaasPackageTier | null
        ...
        notifyAsync(async () => {
          ...
          if (!tenant) return
          const businessName = (tenant.brand_config as { business_name?: string } | null)?.business_name ?? undefined
          const email        = (tenant.brand_config as { contact?: { email?: string | null } } | null)?.contact?.email ?? undefined
          await sendTenantNotification({
            ...
            data: {
              ...
              planInterval: interval === 'year' ? 'annual' : interval === 'month' ? 'monthly' : undefined,
      ...
      case 'customer.subscription.updated': {
        ...
        const oldTier      = (tenantBefore?.package_tier ?? null) as WaasPackageTier | null

        // Derive new tier and interval from subscription items
        const priceId  = sub.items.data[0]?.price?.id ?? null
        const newTier  = tierFromPriceId(priceId)
        const newInterval = sub.items.data[0]?.price?.recurring?.interval === 'year'
          ? 'year' as const
        ...
        console.log(`[WaaS Webhook] subscription.updated → tenant ${tenantId} updated=${updated} status=${sub.status} tier=${newTier ?? 'unchanged'}`)
          ...
          notifyAsync(async () => {
            ...
            await sendTenantNotification({
              ...
              data: {
                ...
                planInterval: newInterval === 'year' ? 'annual' : newInterval === 'month' ? 'monthly' : undefined,
      ...
      case 'invoice.payment_failed': {
        ...
        if (customerId) {
          notifyAsync(async () => {
            ...
            const businessName = (tenant.brand_config as { business_name?: string } | null)?.business_name ?? undefined
            const email        = (tenant.brand_config as { contact?: { email?: string | null } } | null)?.contact?.email ?? undefined
            const tier         = (tenant.package_tier as WaasPackageTier | null) ?? null
            await sendTenantNotification({
              type:     'payment_failed',
              tenantId: tenant.id,
              data: {
