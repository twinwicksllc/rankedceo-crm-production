...
export async function PATCH(
  ...
  try {
    ...
    if (body.brand_config !== undefined)
      update.brand_config = body.brand_config;
    if (body.package_tier !== undefined)
      update.package_tier = body.package_tier;
    if (body.status !== undefined) update.status = body.status;
    if (body.target_industry !== undefined)
      update.target_industry = body.target_industry ?? null;
    if (body.target_location !== undefined)
      update.target_location = body.target_location ?? null;
    if (body.crm_account_id !== undefined)
    ...
    try {
      ...
      return NextResponse.json(
        { error: pgErr?.message ?? "Update failed" },
        { status: 500 },
      );
    }
  } catch (err) {
