...
export default function PlumbingSignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: signUpError } = await supabase.auth.signUp({
        email, password,
        options: {
          data: { industry: 'plumbing', product: 'plumb_pro', company_name: companyName },
      ...
      console.error('[PlumbingSignup] Error:', err.message)
      ...
      <Card className="w-full max-w-md border-teal-200 shadow-lg">
        <CardHeader className="space-y-1 text-center">
          <div className="flex justify-center mb-4">
            <IndustryLogo industry="plumbing" height={126} priority />
          </div>
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="companyName" value={companyName} onChange={e => setCompanyName(e.target.value)}
                placeholder="Smith Plumbing Co." className="border-teal-200 focus:ring-teal-500" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="password" type="password" value={password} onChange={e => setPassword(e.target.value)}
                placeholder="Min. 8 characters" required className="border-teal-200 focus:ring-teal-500" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="confirmPassword">Confirm Password</Label>
              <Input id="confirmPassword" type="password" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)}
                placeholder="Repeat your password" required className="border-teal-200 focus:ring-teal-500" />
            </div>
            <Button type="submit" disabled={loading || !isReady} className="w-full bg-teal-600 hover:bg-teal-700 text-white">
              {loading ? 'Creating account...' : 'Create Plumb Pro Account'}
            </Button>
          </form>
        </CardContent>
        <CardFooter className="flex flex-col gap-2 text-center text-sm text-gray-500">
          ...
          <p className="text-xs text-gray-400">This account is for Plumb Pro operators only.</p>
        </CardFooter>
      </Card>
    </div>
  )
