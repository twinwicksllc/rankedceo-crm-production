...
const STATUS_CONFIG = {
  pass:               { emoji: '✅', label: 'Pass',               class: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' },
  pass_with_findings: { emoji: '⚠️',  label: 'Pass w/ findings',  class: 'bg-amber-500/10 text-amber-400 border-amber-500/20'     },
  error:              { emoji: '❌', label: 'Error',              class: 'bg-red-500/10 text-red-400 border-red-500/20'            },
  critical_halt:      { emoji: '🚨', label: 'Critical Halt',     class: 'bg-fuchsia-500/10 text-fuchsia-400 border-fuchsia-500/20'},
  running:            { emoji: '🔄', label: 'Running',           class: 'bg-blue-500/10 text-blue-400 border-blue-500/20'         },
} as const
  ...
  return (
    <div data-testid="qa-reports-page">
      ...
      <div className="mb-8 flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-white" data-testid="qa-reports-heading">
            QA Agent — Run History
          </h1>
          <p className="text-white/40 mt-1 text-sm">
            Weekly full lifecycle runs + PR smoke tests. All records tagged{' '}
            <code className="text-white/30 bg-white/5 px-1.5 py-0.5 rounded text-xs">qa_agent_*</code>
          </p>
        </div>
      ...
      {!runs || runs.length === 0 ? (
        ...
        <div className="rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl overflow-hidden" data-testid="qa-runs-table">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 text-white/40 text-xs">
                <th className="px-5 py-3 text-left font-medium">Status</th>
                <th className="px-5 py-3 text-left font-medium">Scenario</th>
                <th className="px-5 py-3 text-left font-medium hidden md:table-cell">Mode</th>
                <th className="px-5 py-3 text-left font-medium hidden lg:table-cell">Steps</th>
                <th className="px-5 py-3 text-left font-medium hidden lg:table-cell">Duration</th>
                <th className="px-5 py-3 text-left font-medium hidden xl:table-cell">Run ID</th>
              ...
              {runs.map((run) => {
                ...
                return (
                  <tr key={run.id} className="hover:bg-white/[0.02] transition-colors">
                    ...
                    <td className="px-5 py-3 text-white/80 font-medium">{run.scenario}</td>
                    ...
                    <td className="px-5 py-3 hidden xl:table-cell">
                      <code className="text-[10px] text-white/30">{run.run_id.slice(0, 24)}</code>
                    </td>
