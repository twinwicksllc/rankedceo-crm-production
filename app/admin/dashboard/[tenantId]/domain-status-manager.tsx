...
export function DomainStatusManager({ request }: Props) {
  ...
  const handleSave = async () => {
    setLoading(true);
    const result = await updateDomainStatus(
      request.id,
      newStatus,
      notes || undefined,
    );
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-white/30 text-xs">
            {PRIORITY_LABELS[request.priority - 1]}
          </span>
        </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                <path
                  d="M8.5 1.5L10.5 3.5l-7 7H1.5v-2l7-7z"
                  stroke="currentColor"
                  strokeWidth="1.25"
                  strokeLinecap="round"
        ...
        <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
          <div>
            ...
            <select
              value={newStatus}
              onChange={(e) =>
                setNewStatus(e.target.value as WaasDomainRequest["status"])
              }
