...
function StatusBadge({ status }: { status: WaasTenant["status"] }) {
  const config = {
    ...
    cancelled: {
      label: "Cancelled",
      class: "bg-white/5 text-white/30 border-white/10",
    },
  };
  const c = config[status] ?? config.onboarding;
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${c.class}`}
    >
...
export default async function AdminDashboardPage({
  ...
  const [tenantsResult, statsResult, archivedResult, revenueResult] =
    await Promise.all([
      getAdminTenants(),
      getAdminStats(),
  ...
  const tenantsLoadError = tenantsResult.success
    ? null
    : (tenantsResult.error ?? "Unable to load tenant queue data.");
  const archivedTenants = (archivedResult.data ??
    []) as ArchivedTenantListItem[];

  const pendingAllCount = tenants.filter(
    (t) => t.status === "pending_review" || t.status === "onboarding",
  ).length;
  ...
  const selectedCount = tenants.filter((t) =>
    Boolean(t.client_selected_template_slug),
  ).length;
  const awaitingCount = tenants.length - selectedCount;

  const filteredTenants = tenants.filter((tenant) => {
    if (reviewFilter === "selected")
      return Boolean(tenant.client_selected_template_slug);
    if (reviewFilter === "awaiting")
      return !tenant.client_selected_template_slug;
    return true;
  });

  const pending = filteredTenants.filter(
    (t) => t.status === "pending_review" || t.status === "onboarding",
  );
  ...
  return (
    <div data-testid="admin-dashboard-root">
      ...
      <div className="mb-8 flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 mt-1 text-sm">
            Manage all RankedCEO tenants and deployments.
          </p>
        </div>
      ...
      <div
        className="mb-10 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-xl p-5"
        data-testid="waas-clients-table"
      >
                ...
                >
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-white/40">
                      {tenant.submitted_by_email ?? "No email"} • Archived{" "}
                      {new Date(tenant.deleted_at).toLocaleString()}
                    </p>
                  </div>
                  <form
                    action={async () => {
            ...
            {[
              ...
              <Link
                ...
                href={
                  item.key === "all"
                    ? "/admin/dashboard"
                    : `/admin/dashboard?review=${item.key}`
                }
        ...
        {active.length === 0 ? (
          <div className="rounded-2xl bg-white/3 border border-dashed border-white/10 p-10 flex flex-col items-center justify-center gap-2">
            ...
            <p className="text-white/30 text-sm">
              No active sites yet. Deploy a pending tenant to see it here.
            </p>
          </div>
        ) : (
          ...
          <tbody className="divide-y divide-white/5">
            {tenants.map((t) => (
              ...
              >
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div
                      ...
                      style={{
                        background: `linear-gradient(135deg, ${t.brand_config?.colors?.primary ?? "#2563EB"}, ${t.brand_config?.colors?.secondary ?? "#1E40AF"})`,
                      }}
                    >
                ...
                <td className="px-5 py-4">
                  <span className="capitalize text-white/60 text-sm">
                    {t.package_tier}
                  </span>
                </td>
                <td className="px-5 py-4">
