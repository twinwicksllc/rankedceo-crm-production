...
export default async function BillingPage() {
  ...
  const { data: subscriptions } = await subscriptionService.getAllSubscriptions(
    user.id,
  );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Manage your plans, payment methods, and invoices
        </p>
      </div>
          ...
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <div className="flex flex-wrap gap-3 justify-center">
                {(["hvac", "plumbing", "electrical", "smile"] as const).map(
                  (industry) => (
                    <a
                      key={industry}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <span className="text-2xl">
                        {industryEmoji[sub.industry]}
                      </span>
                      <div>
                        <CardTitle className="text-lg">
                          {planInfo?.name || sub.plan_name}
                        </CardTitle>
                        <CardDescription>
                          {sub.billing_interval === "monthly"
                            ? "Monthly"
                            : "Annual"}{" "}
                          plan · ${(sub.amount_cents / 100).toFixed(2)}/
                          {sub.billing_interval === "monthly" ? "mo" : "yr"}
                        </CardDescription>
                      </div>
                    </div>
                    <span
                      className={`text-xs font-semibold px-3 py-1 rounded-full capitalize ${statusColors[sub.status] || "bg-gray-100 text-gray-600"}`}
                    >
                  ...
                  {sub.billing_interval === "monthly" && (
                    <div className="bg-blue-50 border border-blue-100 rounded-lg p-3 mb-4">
                      <p className="text-sm text-blue-700">
                        💡 <strong>Save money</strong> — switch to annual
                        billing and save up to 15%
                      </p>
                      <a
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <CardTitle className="text-base">
                            {PLAN_INFO[sub.industry]?.name || sub.plan_name}
                          </CardTitle>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Update payment methods, download invoices, and manage your billing
            details
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Subscribe to additional industry products
          </CardDescription>
        </CardHeader>
        <CardContent>
