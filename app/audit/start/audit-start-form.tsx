...
function AuditStartFormContent() {
  ...
  return (
    <main className={`relative min-h-screen overflow-hidden ${isLight ? 'bg-[#eef2f6]' : 'bg-[#020b2c]'}`}>
      ...
      {/* Minimal header */}
      <header className={`relative border-b ${isLight ? 'border-slate-200 bg-white/80' : 'border-white/10 bg-[#071634]/80'} backdrop-blur-sm`}>
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          <Link href="/" className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${isLight ? 'bg-cyan-100 text-cyan-700' : 'bg-cyan-500/15 text-cyan-300'}`}>
                Audit
              </span>
            </div>
          </Link>
          ...
          <form onSubmit={submitAudit} className="space-y-5">
            ...
            <div>
              ...
              >
                ...
                Competitor Websites
                <span className={`text-sm font-medium ${isLight ? 'text-slate-500' : 'text-slate-400'}`}>
                  (up to 3)
                </span>
              </label>
            ...
            {error ? (
              <p className={`text-sm font-medium ${isLight ? 'text-rose-600' : 'text-rose-300'}`}>{error}</p>
            ) : null}
