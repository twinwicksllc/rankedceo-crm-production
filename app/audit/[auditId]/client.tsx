...
function AuditReportClientContent({
  audit: initialAudit,
  userEmail,
  userName,
  ...
  useEffect(() => {
    ...
    audit.status,
    audit.manual_review,
    reportData?.data_unavailable_reason,
  ]);
    ...
    return (
      <PageShell>
        ...
        >
          ...
          <h2
            style={{ color: "#9CA3AF", margin: "0 0 12px", fontSize: "1.5rem" }}
          >
            Report Expired
          </h2>
...
function FullReport({
  ...
  const maxTrackedPosition =
    summary?.max_tracked_position ??
    providerMeta?.keyword_max_tracked_position ??
    100;
  const unrankedPositionValue =
    summary?.unranked_position_value ??
    providerMeta?.keyword_unranked_position_value ??
    maxTrackedPosition + 1;
  ...
  return (
    ...
    >
      ...
      {leaderboard.length > 0 && (
        <Section
          title="🏆 Google Ranking Leaderboard"
          subtitle={`Who dominates "${primaryKeyword}" in your area`}
        >
          <RankingLeaderboard
            entries={leaderboard}
            keyword={primaryKeyword}
      ...
      {summary && (
        <Section
          ...
          subtitle="How your site performs across key ranking factors"
        >
      ...
      {/* ── KEYWORDS USED ────────────────────────────────────────────────── */}
      <Section
        title="🧩 Keywords Evaluated"
        subtitle="The exact search queries used for this ranking audit"
      >
      ...
      {/* ── KEYWORD PERFORMANCE ───────────────────────────────────────────── */}
      {summary &&
        (summary.top_search_result ||
          summary.bottom_search_result ||
          summary.mean_position !== null) && (
          <Section
            title="🔎 Keyword Performance"
            subtitle="Best term, weakest term, and average position across the top 5 keywords"
          >
      ...
      {pageSpeed && (
        <Section
          ...
          subtitle="Speed kills — or converts. Here's where you stand."
        >
      ...
      {gapAnalysis && (
        <Section
          ...
          subtitle="Keywords your competitors are using to steal your leads"
        >
          <GapAnalysis
      ...
      {competitors.length > 0 && (
        <Section
          ...
          subtitle="Individual breakdown of each competitor site"
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {competitors.map((url, i) => {
              const domain = extractDomain(url);
              const lbEntry = leaderboard.find(
                (e) => !e.isTarget && e.domain === domain,
              );
              const compData = report.competitors?.find(
              ...
              return (
                <CompetitorCard
                  ...
                  competitor={{
                    ...
                    keywordsRanking: compData
                      ? compData.keywords_ranking
                      : null,
                    topKeywords:
                      compData?.top_keywords ??
                      (lbEntry?.bestPosition ? [primaryKeyword] : []),
                  }}
                  targetDomain={targetDomain}
                  targetScore={score}
                  targetGrade={grade}
      ...
      {report.technical_issues && report.technical_issues.length > 0 && (
        <Section
          ...
          subtitle="Issues found that impact your search rankings"
        >
      ...
      {report.opportunities && report.opportunities.length > 0 && (
        <Section
          ...
          subtitle="Highest-impact actions to improve your rankings fast"
        >
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          (Performance {performanceScore} × 40%) + (SEO {seoScore} × 30%) +
          (Mobile {mobileScore} × 20%) + (Accessibility {accessibilityScore} ×
          10%) = {weightedRaw.toFixed(1)} → {score}
        </div>
        <div
          style={{
            marginTop: 4,
        ...
        >
          Ranking visibility is shown separately: {measuredKeywords}/
          {evaluatedKeywords} evaluated keywords currently ranking in the top{" "}
          {maxTrackedPosition}.
        </div>
      </div>
      ...
      >
        ...
        <div style={{ flex: 1, minWidth: 240 }}>
          ...
          >
            <span style={{ color: "#EF4444" }}>{targetDomain}</span> vs.
            Competitors
          </h1>
          ...
          >
            ...
            <em
              style={{
                color: isLight ? "rgba(15,23,42,0.9)" : "rgba(255,255,255,0.7)",
              }}
            >
              "{keyword}"
            </em>{" "}
          ...
          >
            <ScorePill
              ...
              glow
            />
            <ScorePill label="Grade" value={grade} color={gradeColor} />
            {isUrgent && (
              <div
                style={{
                  padding: "4px 12px",
...
function ScoreBreakdown({
  ...
  const { theme } = useOnboardingTheme();
  const isLight = theme === "light";
  const performanceContribution = summary.performance_score * 0.4;
  const seoContribution = summary.seo_score * 0.3;
  const mobileContribution = summary.mobile_score * 0.2;
  const accessibilityContribution = summary.accessibility_score * 0.1;
  const rawOverall =
...
function formatPosition(
  ...
  return position > maxTrackedPosition
    ? `Not ranked (Top ${maxTrackedPosition})`
    : `#${position}`;
}
...
function KeywordPerformancePanel({
  summary,
  providerMeta,
  maxTrackedPosition,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      >
        <strong>Ranking Window:</strong> {serpDepthLabel} returned results per
        keyword for this run. Rankings are evaluated against top{" "}
        {maxTrackedPosition} for this report.
      </div>
      <div
        style={{
          fontSize: "0.75rem",
      ...
      >
        Evaluated {evaluated} high-intent keywords. Unranked terms are treated
        as position {unrankedPositionValue} (outside top {maxTrackedPosition})
        in the mean.
      </div>
      <div
        style={{
    ...
    <div>
      ...
      >
        ...
        >
          <VitalMetric
            label="LCP"
            value={formatMs(mobile.lcp)}
            good={mobile.lcp < 2500}
            warn={mobile.lcp < 4000}
            desc="Largest Contentful Paint"
          />
          <VitalMetric
            label="FID"
            value={formatMs(mobile.fid)}
            good={mobile.fid < 100}
            warn={mobile.fid < 300}
          ...
          <VitalMetric
            ...
            good={mobile.cls < 0.1}
            warn={mobile.cls < 0.25}
            desc="Cumulative Layout Shift"
          />
          <VitalMetric
            label="TTFB"
            value={formatMs(mobile.ttfb)}
            good={mobile.ttfb < 800}
            warn={mobile.ttfb < 1800}
...
function VitalMetric({
  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: "0.62rem", color }}>{status}</div>
    </div>
  );
}
...
function TechnicalIssues({
  ...
  issues: NonNullable<AuditReportData["technical_issues"]>;
}) {
  const { theme } = useOnboardingTheme();
  const isLight = theme === "light";
  ...
  const explainIssue = (type: string): { why: string; action: string } => {
    ...
    if (
      normalized.includes("crawlable-anchor") ||
      normalized.includes("crawlable")
    ) {
      return {
        ...
        action:
          "We replace or fix blocked/non-standard links so search bots can follow your key service and location pages.",
      };
    }
    ...
    if (normalized.includes("image-alt") || normalized.includes("alt")) {
      return {
        ...
        action:
          "We add descriptive, keyword-relevant alt text to important images and templates without keyword stuffing.",
      };
    }
    ...
    if (normalized.includes("meta-description")) {
      return {
        ...
        action:
          "We rewrite compelling meta descriptions to improve click-through and match local search intent.",
      };
    }
    ...
    if (normalized.includes("document-title")) {
      return {
        ...
        action:
          "We rewrite title tags for key pages with clear local-intent targeting and proper length.",
      };
    }
    ...
    if (normalized.includes("structured-data")) {
      return {
        ...
        action:
          "We implement or correct schema markup for organization, services, and local business entities.",
      };
    }
    ...
    return {
      ...
      action:
        "We apply a targeted technical fix, then re-test to confirm the issue is resolved and rankings can improve.",
    };
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
      ...
      >
        ...
        >
          Each issue below includes why it affects rankings and exactly what
          we'll do differently for your site to fix it.
        </div>
      </div>
...
function Opportunities({
  ...
  opportunities: NonNullable<AuditReportData["opportunities"]>;
}) {
  const { theme } = useOnboardingTheme();
  const isLight = theme === "light";
  const IMPACT_CONFIG = {
...
function ReportFooter({
  ...
  return (
    ...
    >
      ...
      >
        © {new Date().getFullYear()} RankedCEO · Powered by Surface Audit Engine
        v2 ·{" "}
        <a
          href="/"
          style={{
            color: isLight ? "rgba(15,23,42,0.55)" : "rgba(255,255,255,0.3)",
            textDecoration: "none",
          }}
        >
          rankedceo.com
