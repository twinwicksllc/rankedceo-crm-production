...
function ScorePill({ label, score }: { label: string; score: number | null }) {
  return (
    <div className="flex flex-col items-center gap-0.5">
      <span className={`text-lg font-extrabold leading-none ${scoreColor(score)}`}>
        {score !== null ? score : '–'}
      </span>
...
function EmptyState() {
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center px-4">
      ...
      <h3 className="text-base font-semibold text-slate-700 mb-1">No audits yet</h3>
      <p className="text-sm text-slate-400 max-w-xs">
        Your SEO audit history will appear here once audits have been completed for your site.
      </p>
    </div>
  )
}
...
function AuditRow({ item }: { item: TenantAuditHistoryItem }) {
  ...
  const displayUrl = item.targetUrl.replace(/^https?:\/\//, '').replace(/\/$/, '').slice(0, 45)
  const urlTruncated = item.targetUrl.replace(/^https?:\/\//, '').replace(/\/$/, '').length > 45
  ...
  return (
    <div className={`bg-white rounded-xl border p-4 hover:shadow-sm transition-shadow ${
      ...
      <div className="flex items-start gap-4">
        ...
        <div className={`shrink-0 w-12 h-12 rounded-xl flex flex-col items-center justify-center ${scoreBg(overall)}`}>
          <span className={`text-lg font-extrabold leading-none ${scoreColor(overall)}`}>
            {failed ? '!' : scoreGrade(overall)}
          </span>
          {!failed && overall !== null && (
            <span className={`text-[9px] font-semibold ${scoreColor(overall)}`}>{overall}</span>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-2">
            <div className="min-w-0">
              <p className="text-sm font-semibold text-slate-800 truncate" title={item.targetUrl}>
                {displayUrl}{urlTruncated ? '…' : ''}
              </p>
              <p className="text-xs text-slate-400 mt-0.5">
                {formatDate(item.completedAt)}
                {item.auditType === 'prospect' && (
                  <span className="ml-2 text-[10px] font-medium text-blue-500 bg-blue-50 px-1.5 py-0.5 rounded">Prospect</span>
                )}
              </p>
            </div>
            {!failed && (
          ...
          {failed && (
            <p className="text-xs text-red-500">Audit failed — please contact support if this persists.</p>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-50">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {/* Page title */}
        <div>
          <h2 className="text-base font-semibold text-slate-800">Audit History</h2>
        ...
        {completedItems.length > 0 && (
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
              ...
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400 mt-0.5">Best score</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
              <p className="text-2xl font-extrabold text-slate-800">{completedItems.length}</p>
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400 mt-0.5">Audits run</p>
            </div>
            <div className="bg-white rounded-xl border border-slate-200 p-4 text-center">
              <p className={`text-2xl font-extrabold ${scoreColor(completedItems[0]?.overallScore ?? null)}`}>
                {completedItems[0]?.overallScore ?? '–'}
              </p>
              <p className="text-[10px] font-medium uppercase tracking-wider text-slate-400 mt-0.5">Latest score</p>
            </div>
          </div>
        )}
        ...
        {failedItems.length > 0 && completedItems.length > 0 && (
          <p className="text-xs text-center text-slate-400">
            {failedItems.length} audit{failedItems.length > 1 ? 's' : ''} failed.{' '}
