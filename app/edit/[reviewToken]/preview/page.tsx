...
import type {
  ResolvedTenant, BrandConfig, TenantSiteConfig, SectionConfig,
...
async function loadPreviewData(session: {
  ...
  const templateSlug = session.selectedTemplateSlug
    ?? configRow?.site_templates?.slug
    ?? 'modern'
  ...
  if (session.selectedVariantIndex != null) {
    ...
    const variantSections = (variantRow as { sections_json?: unknown } | null)?.sections_json
...
export default async function ClientEditorPreviewPage({ params }: PreviewProps) {
