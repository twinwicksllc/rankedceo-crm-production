...
export function PlanCard({
  ...
  const { packageTier, planInterval, hasActiveSubscription, planDisplay } =
    billingStatus;
  ...
  function handleUpgrade(tier: Exclude<WaasPackageTier, "hosting">) {
    ...
    const effectiveInterval =
      tier === "hosting_only" ? "year" : upgradeInterval;
    startTransition(async () => {
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 py-3.5 border-b border-slate-100">
        ...
        <span
          className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${planColors[packageTier]}`}
        >
          {planDisplay.label}
          {planInterval && (
            <span className="opacity-70">
              · {planInterval === "month" ? "Monthly" : "Annual"}
            </span>
          )}
        </span>
      </div>
        ...
        {currentTierIdx < tierIndex("premium") && (
          <div className="pt-2 border-t border-slate-100">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              >
                Annual
                <span className="ml-1 text-[10px] text-emerald-500 font-semibold">
                  Save 15%
                </span>
              </button>
            </div>
            ...
            <div className="space-y-2">
              {(
                ...
                ) as Exclude<WaasPackageTier, "hosting">[]
              ).map((tier) => {
