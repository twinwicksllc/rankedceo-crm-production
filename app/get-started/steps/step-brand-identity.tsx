...
const PALETTE_CATEGORIES: PaletteCategory[] = [
  {
    ...
    icon: '🔵',
    palettes: [
      { primary: '#1B4F9C', secondary: '#FF7A00', label: 'Pro Fleet',      mood: 'HVAC · Plumbing · Electric' },
      { primary: '#255FDC', secondary: '#F2C300', label: 'Service Pro',     mood: 'Plumbing · HVAC · Roofing'  },
      { primary: '#375A7F', secondary: '#2CA6A4', label: 'Slate & Teal',   mood: 'Remodeling · Bath · Kitchen' },
      { primary: '#0A2342', secondary: '#C0D6F5', label: 'Navy Command',   mood: 'Commercial · Industrial GC' },
      { primary: '#1A5276', secondary: '#85C1E9', label: 'Steel Blue',     mood: 'Plumbing · Water Services'  },
      { primary: '#2E86C1', secondary: '#1A5276', label: 'Corporate Blue', mood: 'Multi-trade · General Contractor' },
    ],
  },
  {
    id:   'bold',
    ...
    icon: '🟠',
    palettes: [
      { primary: '#D35400', secondary: '#2C3E50', label: 'Job Site',       mood: 'General Contractor · Framing' },
      { primary: '#E74C3C', secondary: '#2C3E50', label: 'Power Red',      mood: 'Roofing · Demolition · Concrete' },
      { primary: '#F39C12', secondary: '#1A252F', label: 'Safety Gold',    mood: 'Exterior · Solar · Service Vans' },
      { primary: '#CB4335', secondary: '#922B21', label: 'Bold Crimson',   mood: 'Emergency Service · Fire/Alarm' },
      { primary: '#BA4A00', secondary: '#1F618D', label: 'Copper & Navy',  mood: 'Roofing · Metalwork · HVAC'  },
      { primary: '#E67E22', secondary: '#154360', label: 'High-Vis Pro',   mood: 'Construction · Traffic · Crew' },
    ],
  },
  {
    id:   'premium',
    ...
    icon: '⚫',
    palettes: [
      { primary: '#1C1B1A', secondary: '#B08D57', label: 'Warm Black & Brass', mood: 'Custom Home · High-End Finishes' },
      { primary: '#212F3D', secondary: '#C9A84C', label: 'Midnight Gold',  mood: 'Design-Build · Custom Millwork' },
      { primary: '#1A1A2E', secondary: '#E94560', label: 'Urban Premium',  mood: 'Modern · Boutique · Tech-Forward' },
      { primary: '#2D2D2D', secondary: '#B7950B', label: 'Charcoal & Gold', mood: 'Architectural · Specialty Finishes' },
      { primary: '#4A235A', secondary: '#D4AC0D', label: 'Eggplant & Brass', mood: 'Design-Build · Luxury Renovation' },
      { primary: '#17202A', secondary: '#85929E', label: 'Slate Prestige', mood: 'Commercial · Steel · Precision Work' },
    ],
  },
  {
    id:   'nature',
    ...
    icon: '🌿',
    palettes: [
      { primary: '#0E2A21', secondary: '#C96F53', label: 'Forest & Clay',  mood: 'Landscape · Outdoor Living'    },
      { primary: '#1D4E2E', secondary: '#E6DDCC', label: 'Deep Forest',    mood: 'Landscaping · Green Building'  },
      { primary: '#2E7D32', secondary: '#F9A825', label: 'Growth & Sun',   mood: 'Solar · Eco-Build · Sustainable'},
      { primary: '#4E342E', secondary: '#A5D6A7', label: 'Walnut & Sage',  mood: 'Flooring · Custom Woodwork'    },
      { primary: '#5D4037', secondary: '#FFCC02', label: 'Timber & Gold',  mood: 'Framing · Stone · Custom Homes' },
      { primary: '#37474F', secondary: '#80CBC4', label: 'Concrete & Teal', mood: 'Epoxy · Tile · Modern Spaces'  },
    ],
  },
]
...
export function StepBrandIdentity({
  ...
  const handleFileSelect = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!validTypes.includes(file.type)) { setUploadError('Please upload a JPG, PNG, SVG, or WebP image.'); return }
    ...
    try {
      ...
      const response = await fetch('/api/onboarding/logo-upload', { method: 'POST', body: formData })
      const payload  = await response.json().catch(() => ({} as { error?: string; publicUrl?: string; success?: boolean }))
      if (!response.ok || !payload.success || !payload.publicUrl) throw new Error(payload.error ?? 'Upload failed')
  ...
  const handleRemoveLogo = () => { setLogoUrl(null); setUseAutoLogo(true); setLogoColorsSuggested(false) }
  ...
  const currentCategory = PALETTE_CATEGORIES.find(c => c.id === activeCategory)!
    ...
    <div>
      ...
      <div className="mb-8">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          >
            <input ref={fileRef} type="file" accept="image/jpeg,image/png,image/svg+xml,image/webp"
          ...
          >
            ...
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={svgDataUrl} alt="Auto-generated text-mark" className="max-h-10 max-w-full object-contain" />
            <div className="text-center">
              <p className="text-slate-600 dark:text-white/60 text-sm font-medium">Auto-Generate</p>
              <p className="text-slate-400 dark:text-white/30 text-xs mt-0.5">Professional text-mark</p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-xs text-slate-400 dark:text-white/30">
            {logoColorsSuggested ? '✨ Colors suggested from your logo' : 'Pick a palette or customize below'}
          </span>
        </div>
              ...
              >
                ...
                <div className="px-2.5 py-2 bg-white dark:bg-slate-900/60">
                  <div className="flex items-center justify-between gap-1">
                    <p className="text-xs font-semibold text-slate-800 dark:text-white/90 truncate">{p.label}</p>
                    {isSelected && (
                  ...
                  <p className="text-[10px] text-slate-400 dark:text-white/35 truncate mt-0.5 leading-tight">{p.mood}</p>
                </div>
              </button>
            )
          })}
        ...
        <div className="p-4 rounded-xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/10">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-white/50 mb-1.5">Primary Color</label>
              <div className="flex items-center gap-3 h-11 px-3 rounded-xl bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 focus-within:border-blue-500/60 transition-all">
                ...
                />
                <span className="text-slate-600 dark:text-white/60 text-xs font-mono uppercase">{primaryColor}</span>
                <span className="ml-auto text-[10px] text-slate-400 dark:text-white/25 hidden sm:block">auto-pairs secondary</span>
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-600 dark:text-white/50 mb-1.5">Secondary / Accent Color</label>
              <div className="flex items-center gap-3 h-11 px-3 rounded-xl bg-white dark:bg-white/5 border border-slate-300 dark:border-white/10 focus-within:border-blue-500/60 transition-all">
                <input
                  type="color"
                  ...
                  className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0 outline-none p-0"
                />
                <span className="text-slate-600 dark:text-white/60 text-xs font-mono uppercase">{secondaryColor}</span>
              </div>
            </div>
          </div>
          {logoColorsSuggested && (
        ...
        <div className="mt-4 p-4 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10">
          ...
          <div className="space-y-2.5">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="h-8 w-8 rounded-lg border-2 shrink-0"
